(function (global, factory) {
  ...
  class PivotProcessor {
      ...
      const sortGroup = (values, level) => {
        ...
        values.sort((a, b) => {
          ...
          if (sortBy === 'label') {
            ...
            const valueField = this.config.values.find(v => {
              const vfId = v.id || `${v.field}_${v.aggregation.toLowerCase()}`;
              return vfId === sortBy;
            });
            if (valueField) {
    ...
    generateRowHierarchy(rowFields) {
      ...
      this.rawData.forEach(record => {
        ...
        rowFields.forEach((field, depth) => {
          ...
          currentLevel.get(value).records.push(record);
          currentLevel = currentLevel.get(value).children;
        });
      });

      // Apply sorting at each level
      const sortLevel = (map, field) => {
        ...
        entries.sort(([keyA, nodeA], [keyB, nodeB]) => {
          ...
          if (sortBy === 'label') {
            ...
            // Find the value field configuration
            const valueField = this.config.values.find(v => {
              const vfId = v.id || `${v.field}_${v.aggregation.toLowerCase()}`;
              return vfId === sortBy;
            });
            if (valueField) {
              // Calculate aggregated values for comparison using the same aggregation function
              valueA = this.aggregateValues(nodeA.records, valueField.field, valueField.aggregation);
              valueB = this.aggregateValues(nodeB.records, valueField.field, valueField.aggregation);
            } else {
              // Fallback to label sorting if value field not found
              valueA = keyA;
              valueB = keyB;
            }
          }
        ...
        entries.forEach(([key, node]) => {
          ...
          if (node.children.size > 0 && rowFields[node.depth + 1]) {
            node.children = sortLevel(node.children, rowFields[node.depth + 1]);
          }
          sortedMap.set(key, node);
        });
        return sortedMap;
      };

      // Start sorting from the top level
      return this.mapToArray(sortLevel(hierarchy, rowFields[0]));
    }
    ...
    generateHeaders(colValues, colFields, valueFields) {
      ...
      colValues.forEach(cv => {
        valueFields.forEach(vf => {
          const valueFieldId = vf.id || `${vf.field}_${vf.aggregation.toLowerCase()}`;
          leafHeaders.push({
            id: `${cv}|${valueFieldId}`,
            label: `${vf.field} (${vf.aggregation})`,
            colspan: 1,
    ...
    generateRows(rowHierarchy, colValues, rowFields, colFields, valueFields) {
      const processLevel = (node, parent = null) => {
        const cells = colValues.length ? colValues.flatMap(cv => {
          const colCriteria = cv.split('|').reduce((acc, val, idx) => {
            acc[colFields[idx].field] = val;
            return acc;
          }, {});

          // For each column value, generate cells for each value field
          return valueFields.map(vf => {
            const filteredRecords = this.filterData(node.records, colCriteria);
            const valueFieldId = vf.id || `${vf.field}_${vf.aggregation.toLowerCase()}`;
            return {
        ...
        row.children = node.children ? node.children.map(child => processLevel(child, row)) : [];
        return row;
      };
      ...
      return {
        ...
        cells: rows[0].cells.map(cell => {
          ...
          const valueField = valueFields.find(vf => {
            const vfId = vf.id || `${vf.field}_${vf.aggregation.toLowerCase()}`;
    ...
    methods: {
      ...
      onDrop(event, targetZone) {
        ...
        if (sourceZone === targetZone) {
          ...
          if (targetZone === 'filters') {
            items = [...this.filters];
            draggedItem = items.find(f => f.column === field);
          } else {
      ...
      addNewFilter() {
        ...
        if (this.filters.push) {
          ...
          this.$set(this, 'filters', [...(this.filters || []), newFilter]);
        }
      ...
      addNewField(zone) {
        ...
        switch (zone) {
          ...
          case 'values': {
            ...
            const newId = `${this.selectedField}_${defaultAggregation}_${this.nextValueId++}`;
      ...
      onAggregationChange(field) {
        ...
        // Update the field's ID to reflect the change
        field.id = `${field.field}_${field.aggregation}_${this.nextValueId++}`;
      ...
      toggleFilterType(filter) {
        ...
        if (filter.filterType === 'values') {
          ...
          this.$set(filter, 'selectedValues', []);
          this.$set(filter, 'searchQuery', '');
        } else {
        ...
        _c(
          "div",
          {
            ...
            class: {
              ...
              "has-no-results": _vm.hasNoResults && _vm.filters.length > 0,
            },
              ...
              [
                _vm._l(_vm.filters, function (filter, index) {
                  return _c(
                    "div",
                    {
                      key: "filter_" + filter.column + "_" + index,
                      staticClass: "field-item dropped",
                      attrs: { draggable: "true" },
                      on: {
                        dragstart: function ($event) {
                          return _vm.onDragStart($event, filter.column, "filters")
                        },
                        dragend: _vm.onDragEnd,
                      },
                    },
                          ...
                          : _c(
                              ...
                              [
                                ...
                                !["isEmpty", "isNotEmpty"].includes(
                                  ...
                                  ? [
                                      _vm.getInputType(filter.column) ===
                                      "checkbox"
                                        ? _c("input", {
                                            directives: [
                                              {
  ...
  __vue_render__$2._withStripped = true;
    ...
    const __vue_inject_styles__$2 = function (inject) {
      if (!inject) return
      inject("data-v-6f40c4a4_0", { source: ".fields-container[data-v-6f40c4a4] {\n  display: flex;\n  flex-direction: column;\n  gap: 16px;\n  width: 300px;\n  background-color: var(--st-dashboard-bg-0);\n  padding: 6px;\n  border-radius: 8px;\n  border: 1px solid var(--st-color-neutral);\n  height: 100%;\n  box-sizing: border-box;\n  position: relative;\n}\n.zones-container[data-v-6f40c4a4] {\n  display: flex;\n  flex-direction: column;\n  gap: 2px;\n  flex: 1;\n  overflow-y: auto;\n  padding-bottom: 12px;\n}\n.zones-container[data-v-6f40c4a4]::-webkit-scrollbar {\n  width: 8px;\n  height: 8px;\n}\n.zones-container[data-v-6f40c4a4]::-webkit-scrollbar-track {\n  background: transparent;\n}\n.zones-container[data-v-6f40c4a4]::-webkit-scrollbar-thumb {\n  background: var(--st-color-neutral);\n  border-radius: 4px;\n}\n.zones-container[data-v-6f40c4a4]::-webkit-scrollbar-thumb:hover {\n  background: color-mix(in srgb, var(--st-color-neutral) 80%, transparent);\n}\n.zones-container[data-v-6f40c4a4]::-webkit-scrollbar-corner {\n  background: transparent;\n}\n.field-item[data-v-6f40c4a4] {\n  position: relative;\n  z-index: 1;\n  padding: 12px 12px 12px 12px;\n  background: #f8f9fa;\n  border: 1px solid #e9ecef;\n  border-radius: 4px;\n  margin-bottom: 8px;\n  cursor: move;\n  display: flex;\n  justify-content: space-between;\n  align-items: flex-start;\n  font-size: 13px;\n}\n.field-item[data-v-6f40c4a4]:last-child {\n  margin-bottom: 0;\n}\n.field-item.dropped[data-v-6f40c4a4] {\n  background: color-mix(in srgb, var(--st-text-2) 5%, transparent);\n  border-color: var(--st-color-neutral);\n  color: var(--st-text-1);\n}\n.has-no-results .field-item.dropped[data-v-6f40c4a4] {\n  background: color-mix(in srgb, #ff0000 15%, transparent);\n  border-color: color-mix(in srgb, #ff0000 35%, transparent);\n}\n.has-no-results .field-item.dropped .filter-controls select[data-v-6f40c4a4], .has-no-results .field-item.dropped .filter-controls input[data-v-6f40c4a4] {\n  border-color: color-mix(in srgb, #ff0000 35%, transparent);\n}\n.has-no-results .field-item.dropped .filter-controls select[data-v-6f40c4a4]:focus, .has-no-results .field-item.dropped .filter-controls input[data-v-6f40c4a4]:focus {\n  border-color: #ef5350;\n}\n.field-item.drag-over-top[data-v-6f40c4a4] {\n  border-top: 2px solid #0066cc;\n}\n.field-item.drag-over-bottom[data-v-6f40c4a4] {\n  border-bottom: 2px solid #0066cc;\n}\n.remove-btn[data-v-6f40c4a4] {\n  position: absolute;\n  top: 2px;\n  right: 2px;\n  background: none;\n  border: none;\n  color: #adb5bd;\n  cursor: pointer;\n  padding: 4px;\n  font-size: 16px;\n  line-height: 1;\n  z-index: 2;\n  border-radius: 4px;\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  width: 20px;\n  height: 20px;\n}\n.remove-btn[data-v-6f40c4a4]:hover {\n  color: #495057;\n  background-color: rgba(0, 0, 0, 0.05);\n}\n.field-content[data-v-6f40c4a4] {\n  display: flex;\n  flex-direction: column;\n  gap: 6px;\n  flex: 1;\n  min-width: 0;\n}\n.section[data-v-6f40c4a4] {\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 4px;\n  background: var(--st-dashboard-bg-1);\n}\n.section .section-header[data-v-6f40c4a4] {\n  padding: 5px 10px;\n  background: var(--st-dashboard-bg-2);\n  border-bottom: 1px solid var(--st-color-neutral);\n  cursor: pointer;\n  display: flex;\n  justify-content: space-between;\n  align-items: center;\n  user-select: none;\n}\n.section .section-header[data-v-6f40c4a4]:hover {\n  background: color-mix(in srgb, var(--st-dashboard-bg-2), var(--st-text-1) 10%);\n}\n.section .section-header h4[data-v-6f40c4a4] {\n  margin: 0;\n  color: var(--st-text-1);\n  font-size: 14px;\n  font-weight: 600;\n}\n.section .section-content[data-v-6f40c4a4] {\n  padding: 7px 5px;\n  transition: max-height 0.3s ease-in-out;\n  overflow: visible;\n  position: relative;\n}\n.section .section-content.collapsed[data-v-6f40c4a4] {\n  max-height: 0;\n  padding: 0;\n  overflow: hidden;\n}\n.header-controls[data-v-6f40c4a4] {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n}\n.add-btn[data-v-6f40c4a4] {\n  background: none;\n  border: none;\n  color: var(--st-text-2);\n  cursor: pointer;\n  padding: 2px 6px;\n  font-size: 16px;\n  line-height: 1;\n  border-radius: 4px;\n}\n.add-btn[data-v-6f40c4a4]:hover {\n  color: var(--st-text-1);\n  background-color: color-mix(in srgb, var(--st-text-1) 10%, transparent);\n}\n.toggle-icon[data-v-6f40c4a4] {\n  font-size: 12px;\n  color: #6c757d;\n  transition: transform 0.3s ease;\n}\n.toggle-icon.collapsed[data-v-6f40c4a4] {\n  transform: rotate(-90deg);\n}\n.available-fields[data-v-6f40c4a4], .drop-zone[data-v-6f40c4a4] {\n  margin-bottom: 16px;\n}\n.available-fields[data-v-6f40c4a4]:last-child, .drop-zone[data-v-6f40c4a4]:last-child {\n  margin-bottom: 0;\n}\n.field-item[data-v-6f40c4a4] {\n  margin-bottom: 8px;\n}\n.field-item[data-v-6f40c4a4]:last-child {\n  margin-bottom: 0;\n}\n.filter-controls[data-v-6f40c4a4] {\n  display: flex;\n  flex-direction: column;\n  gap: 4px;\n  width: 100%;\n}\n.filter-controls select[data-v-6f40c4a4], .filter-controls input[data-v-6f40c4a4] {\n  width: 100%;\n  padding: 4px;\n  border: 1px solid var(--st-color-neutral);\n  background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n  color: var(--st-text-1);\n  border-radius: 3px;\n  font-size: 12px;\n}\n.filter-controls select[data-v-6f40c4a4]:focus, .filter-controls input[data-v-6f40c4a4]:focus {\n  outline: none;\n  border-color: #0066cc;\n}\n.add-filter-section[data-v-6f40c4a4] {\n  padding: 8px;\n  border-top: 1px solid #eee;\n  margin-top: 8px;\n}\n.column-select-container[data-v-6f40c4a4] {\n  display: flex;\n  gap: 8px;\n  align-items: center;\n  width: 100%;\n}\n.column-select[data-v-6f40c4a4] {\n  flex: 1;\n  padding: 6px 8px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n  font-size: 13px;\n  background-color: white;\n}\n.column-select[data-v-6f40c4a4]:focus {\n  outline: none;\n  border-color: #0066cc;\n}\n.cancel-btn[data-v-6f40c4a4] {\n  background: none;\n  border: none;\n  color: #adb5bd;\n  cursor: pointer;\n  padding: 4px;\n  font-size: 16px;\n  line-height: 1;\n  border-radius: 4px;\n}\n.cancel-btn[data-v-6f40c4a4]:hover {\n  color: #495057;\n  background-color: #f8f9fa;\n}\n.add-filter-btn[data-v-6f40c4a4] {\n  width: 100%;\n  padding: 6px 12px;\n  background-color: #f8f9fa;\n  border: 1px dashed #adb5bd;\n  border-radius: 4px;\n  color: #495057;\n  cursor: pointer;\n  font-size: 13px;\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  gap: 4px;\n}\n.add-filter-btn[data-v-6f40c4a4]:hover {\n  background-color: #e9ecef;\n  border-color: #495057;\n}\n.add-filter-btn[data-v-6f40c4a4]::before {\n  content: \"+\";\n  font-size: 16px;\n  line-height: 1;\n  margin-right: 4px;\n}\n.add-field-section[data-v-6f40c4a4] {\n  display: none;\n}\n.field-select-container[data-v-6f40c4a4] {\n  position: absolute;\n  top: 47px;\n  right: 4px;\n  width: calc(100% - 25px);\n  background: var(--st-dashboard-bg-1);\n  border: solid 1px var(--st-color-neutral);\n  border-radius: 4px;\n  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.15);\n  padding: 8px;\n  z-index: 1000;\n  display: flex;\n  flex-direction: column;\n  gap: 8px;\n}\n.field-search[data-v-6f40c4a4] {\n  width: 100%;\n  padding: 6px 8px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n  font-size: 13px;\n  box-sizing: border-box;\n}\n.field-search[data-v-6f40c4a4]:focus {\n  outline: none;\n  border-color: #0066cc;\n}\n.field-select[data-v-6f40c4a4] {\n  width: 100%;\n  border-radius: 4px;\n  font-size: 13px;\n  background-color: var(--st-dashboard-bg-1);\n}\n.field-select.expanded[data-v-6f40c4a4] {\n  max-height: 200px;\n  overflow-y: auto;\n}\n.field-select.expanded option[data-v-6f40c4a4] {\n  padding: 6px 8px;\n  color: var(--st-text-1);\n}\n.field-select.expanded option[data-v-6f40c4a4]:hover {\n  background-color: color-mix(in srgb, var(--st-text-1) 10%, transparent);\n}\n.field-select.expanded option[value=\"\"][data-v-6f40c4a4] {\n  color: #6c757d;\n  font-style: italic;\n}\n.field-select[data-v-6f40c4a4]:focus {\n  outline: none;\n  border-color: #0066cc;\n}\n.cancel-btn[data-v-6f40c4a4] {\n  position: absolute;\n  top: 11px;\n  right: 11px;\n  width: 20px;\n  background: none;\n  border: none;\n  color: #adb5bd;\n  cursor: pointer;\n  padding: 4px;\n  font-size: 16px;\n  line-height: 1;\n  border-radius: 4px;\n}\n.cancel-btn[data-v-6f40c4a4]:hover {\n  color: #495057;\n  background-color: #f8f9fa;\n}\n.field-content[data-v-6f40c4a4] {\n  display: flex;\n  flex-direction: column;\n  gap: 4px;\n  width: 100%;\n}\n.field-controls[data-v-6f40c4a4] {\n  display: flex;\n  gap: 4px;\n  align-items: center;\n}\nselect[data-v-6f40c4a4] {\n  font-size: 12px;\n  padding: 2px 4px;\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 3px;\n  background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n  color: var(--st-text-1);\n}\nselect.sort-select[data-v-6f40c4a4] {\n  width: 110px;\n}\nselect.order-select[data-v-6f40c4a4] {\n  width: 90px;\n}\n.field-header[data-v-6f40c4a4] {\n  display: flex;\n  align-items: center;\n  justify-content: space-between;\n  margin-bottom: 12px;\n}\n.field-header .field-name[data-v-6f40c4a4] {\n  /* font-size: 16px;\n  font-weight: 500; */\n}\n.filter-type-selector[data-v-6f40c4a4] {\n  display: flex;\n  padding: 2px;\n  border-radius: 6px;\n  border: 1px solid var(--st-color-neutral);\n  margin-bottom: 12px;\n}\n.filter-type-btn[data-v-6f40c4a4] {\n  flex: 1;\n  padding: 2px 5px;\n  font-size: 14px;\n  border: none;\n  background: transparent;\n  cursor: pointer;\n  border-radius: 4px;\n  color: var(--st-text-2);\n  transition: all 0.2s ease;\n}\n.filter-type-btn.active[data-v-6f40c4a4] {\n  background: color-mix(in srgb, var(--st-text-1) 15%, transparent);\n  color: var(--st-text-1);\n  font-weight: 500;\n}\n.filter-type-btn[data-v-6f40c4a4]:hover:not(.active) {\n  background: color-mix(in srgb, var(--st-text-1) 8%, transparent);\n}\n.values-filter[data-v-6f40c4a4] {\n  /* background: #fff; */\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 6px;\n  overflow: hidden;\n}\n.values-list[data-v-6f40c4a4] {\n  max-height: 300px;\n  overflow-y: auto;\n}\n.values-list-header[data-v-6f40c4a4] {\n  position: sticky;\n  top: 0;\n  background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n  border-bottom: 1px solid var(--st-color-neutral);\n  padding: 12px;\n  z-index: 1;\n}\n.search-container[data-v-6f40c4a4] {\n  margin-bottom: 12px;\n}\n.values-search[data-v-6f40c4a4] {\n  width: 100%;\n  padding: 8px 12px;\n  border: 1px solid #ddd;\n  border-radius: 6px;\n  font-size: 14px;\n  background: #f8f8f8;\n}\n.values-search[data-v-6f40c4a4]:focus {\n  outline: none;\n  border-color: #0066cc;\n  background: #fff;\n}\n.values-search[data-v-6f40c4a4]::placeholder {\n  color: #999;\n}\n.values-actions[data-v-6f40c4a4] {\n  display: flex;\n  gap: 8px;\n}\n.action-btn[data-v-6f40c4a4] {\n  flex: 1;\n  padding: 2px 6px;\n  font-size: 14px;\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 6px;\n  background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n  cursor: pointer;\n  color: var(--st-text-1);\n  transition: all 0.2s ease;\n}\n.action-btn[data-v-6f40c4a4]:hover {\n  background: color-mix(in srgb, var(--st-text-1) 15%, transparent);\n}\n.values-options[data-v-6f40c4a4] {\n  padding: 8px 12px;\n}\n.value-option[data-v-6f40c4a4] {\n  display: flex;\n  align-items: center;\n  padding: 2px 2px;\n  cursor: pointer;\n  border-radius: 4px;\n  transition: background-color 0.2s ease;\n}\n.value-option[data-v-6f40c4a4]:hover {\n  background: color-mix(in srgb, var(--st-text-1) 15%, transparent);\n}\n.checkbox-wrapper[data-v-6f40c4a4] {\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  width: 20px;\n  height: 20px;\n  margin-right: 12px;\n}\n.checkbox-wrapper input[type=checkbox][data-v-6f40c4a4] {\n  width: 16px;\n  height: 16px;\n  margin: 0;\n  cursor: pointer;\n}\n.value-label[data-v-6f40c4a4] {\n  flex: 1;\n  font-size: 14px;\n  overflow: hidden;\n  text-overflow: ellipsis;\n  white-space: nowrap;\n}\n.values-list[data-v-6f40c4a4]::-webkit-scrollbar {\n  width: 8px;\n}\n.values-list[data-v-6f40c4a4]::-webkit-scrollbar-track {\n  background: #f1f1f1;\n  border-radius: 4px;\n}\n.values-list[data-v-6f40c4a4]::-webkit-scrollbar-thumb {\n  background: #ccc;\n  border-radius: 4px;\n}\n.values-list[data-v-6f40c4a4]::-webkit-scrollbar-thumb:hover {\n  background: #bbb;\n}\n.condition-filter select[data-v-6f40c4a4], .condition-filter input[data-v-6f40c4a4] {\n  width: 100%;\n  /* padding: 8px 12px; */\n  /* border: 1px solid #ddd; */\n  border-radius: 6px;\n  font-size: 14px;\n  margin-bottom: 8px;\n  /* background: #fff; */\n  box-sizing: border-box;\n}\n.condition-filter select[data-v-6f40c4a4]:focus, .condition-filter input[data-v-6f40c4a4]:focus {\n  outline: none;\n  border-color: #0066cc;\n}\n.condition-filter input[data-v-6f40c4a4] {\n  padding: 6px 10px;\n}\n.remove-btn[data-v-6f40c4a4] {\n  position: absolute;\n  top: 2px;\n  right: 2px;\n  background: none;\n  border: none;\n  color: #999;\n  font-size: 18px;\n  cursor: pointer;\n  padding: 4px 8px;\n  border-radius: 4px;\n}\n.remove-btn[data-v-6f40c4a4]:hover {\n  color: #666;\n  background: #f0f0f0;\n}\n.formula-input-container[data-v-6f40c4a4] {\n  margin-top: 8px;\n  width: 100%;\n}\n.formula-input[data-v-6f40c4a4] {\n  width: 100%;\n  padding: 8px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n  font-size: 13px;\n}\n.formula-input[data-v-6f40c4a4]:focus {\n  outline: none;\n  border-color: #0066cc;\n}\n.formula-input.error[data-v-6f40c4a4] {\n  border-color: #dc3545;\n  background-color: #fff3f3;\n}\n.formula-error[data-v-6f40c4a4] {\n  color: #dc3545;\n  font-size: 12px;\n  margin-top: 4px;\n  padding: 4px;\n  background-color: #fff3f3;\n  border-radius: 4px;\n}\n.actions-panel[data-v-6f40c4a4] {\n  position: sticky;\n  bottom: 0;\n  left: 0;\n  right: 0;\n  padding: 12px;\n  background: var(--st-dashboard-bg-0);\n  border-top: 1px solid var(--st-color-neutral);\n  display: flex;\n  justify-content: center;\n  margin: 0 -6px -6px -6px;\n  border-radius: 0 0 8px 8px;\n  flex-shrink: 0;\n  z-index: 10;\n}\n.actions-panel .export-btn[data-v-6f40c4a4] {\n  width: 100%;\n  padding: 8px;\n  background: var(--st-dashboard-bg-2);\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 4px;\n  color: var(--st-text-1);\n  cursor: pointer;\n  font-size: 13px;\n  font-weight: 500;\n}\n.actions-panel .export-btn[data-v-6f40c4a4]:hover {\n  background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n}\n\n/*# sourceMappingURL=st-pivottable-controls.vue.map */", map: {"version":3,"sources":["/Users/daniel/Documents/work/stipple_app/pivot-table/lib/src/components/st-pivottable-controls.vue","st-pivottable-controls.vue"],"names":[],"mappings":"AAwoCA;EACA,aAAA;EACA,sBAAA;EACA,SAAA;EACA,YAAA;EACA,0CAAA;EACA,YAAA;EACA,kBAAA;EACA,yCAAA;EACA,YAAA;EACA,sBAAA;EACA,kBAAA;ACvoCA;AD0oCA;EACA,aAAA;EACA,sBAAA;EACA,QAAA;EACA,OAAA;EACA,gBAAA;EACA,oBAAA;ACvoCA;AD0oCA;EACA,UAAA;EACA,WAAA;ACxoCA;AD2oCA;EACA,uBAAA;ACzoCA;AD4oCA;EACA,mCAAA;EACA,kBAAA;AC1oCA;AD4oCA;EACA,wEAAA;AC1oCA;AD8oCA;EACA,uBAAA;AC5oCA;ADgpCA;EACA,kBAAA;EACA,UAAA;EACA,4BAAA;EACA,mBAAA;EACA,yBAAA;EACA,kBAAA;EACA,kBAAA;EACA,YAAA;EACA,aAAA;EACA,8BAAA;EACA,uBAAA;EACA,eAAA;AC7oCA;AD+oCA;EACA,gBAAA;AC7oCA;ADgpCA;EACA,gEAAA;EACA,qCAAA;EACA,uBAAA;AC9oCA;ADipCA;EACA,wDAAA;EACA,0DAAA;AC/oCA;ADkpCA;EACA,0DAAA;AChpCA;ADkpCA;EACA,qBAAA;AChpCA;ADupCA;EACA,6BAAA;ACrpCA;ADwpCA;EACA,gCAAA;ACtpCA;AD0pCA;EACA,kBAAA;EACA,QAAA;EACA,UAAA;EACA,gBAAA;EACA,YAAA;EACA,cAAA;EACA,eAAA;EACA,YAAA;EACA,eAAA;EACA,cAAA;EACA,UAAA;EACA,kBAAA;EACA,aAAA;EACA,mBAAA;EACA,uBAAA;EACA,WAAA;EACA,YAAA;ACvpCA;ADypCA;EACA,cAAA;EACA,qCAAA;ACvpCA;AD2pCA;EACA,aAAA;EACA,sBAAA;EACA,QAAA;EACA,OAAA;EACA,YAAA;ACxpCA;AD2pCA;EACA,yCAAA;EACA,kBAAA;EACA,oCAAA;ACxpCA;AD0pCA;EACA,iBAAA;EACA,oCAAA;EACA,gDAAA;EACA,eAAA;EACA,aAAA;EACA,8BAAA;EACA,mBAAA;EACA,iBAAA;ACxpCA;AD0pCA;EACA,8EAAA;ACxpCA;AD2pCA;EACA,SAAA;EACA,uBAAA;EACA,eAAA;EACA,gBAAA;ACzpCA;AD6pCA;EACA,gBAAA;EACA,uCAAA;EACA,iBAAA;EACA,kBAAA;AC3pCA;AD6pCA;EACA,aAAA;EACA,UAAA;EACA,gBAAA;AC3pCA;ADgqCA;EACA,aAAA;EACA,mBAAA;EACA,QAAA;AC7pCA;ADgqCA;EACA,gBAAA;EACA,YAAA;EACA,uBAAA;EACA,eAAA;EACA,gBAAA;EACA,eAAA;EACA,cAAA;EACA,kBAAA;AC7pCA;AD+pCA;EACA,uBAAA;EACA,uEAAA;AC7pCA;ADiqCA;EACA,eAAA;EACA,cAAA;EACA,+BAAA;AC9pCA;ADgqCA;EACA,yBAAA;AC9pCA;ADmqCA;EACA,mBAAA;AChqCA;ADkqCA;EACA,gBAAA;AChqCA;ADoqCA;EACA,kBAAA;ACjqCA;ADmqCA;EACA,gBAAA;ACjqCA;ADqqCA;EACA,aAAA;EACA,sBAAA;EACA,QAAA;EACA,WAAA;AClqCA;ADoqCA;EACA,WAAA;EACA,YAAA;EAEA,yCAAA;EACA,gEAAA;EACA,uBAAA;EAEA,kBAAA;EACA,eAAA;ACpqCA;ADsqCA;EACA,aAAA;EACA,qBAAA;ACpqCA;ADyqCA;EACA,YAAA;EACA,0BAAA;EACA,eAAA;ACtqCA;ADyqCA;EACA,aAAA;EACA,QAAA;EACA,mBAAA;EACA,WAAA;ACtqCA;ADyqCA;EACA,OAAA;EACA,gBAAA;EACA,sBAAA;EACA,kBAAA;EACA,eAAA;EACA,uBAAA;ACtqCA;ADwqCA;EACA,aAAA;EACA,qBAAA;ACtqCA;AD0qCA;EACA,gBAAA;EACA,YAAA;EACA,cAAA;EACA,eAAA;EACA,YAAA;EACA,eAAA;EACA,cAAA;EACA,kBAAA;ACvqCA;ADyqCA;EACA,cAAA;EACA,yBAAA;ACvqCA;AD2qCA;EACA,WAAA;EACA,iBAAA;EACA,yBAAA;EACA,0BAAA;EACA,kBAAA;EACA,cAAA;EACA,eAAA;EACA,eAAA;EACA,aAAA;EACA,mBAAA;EACA,uBAAA;EACA,QAAA;ACxqCA;AD0qCA;EACA,yBAAA;EACA,qBAAA;ACxqCA;AD2qCA;EACA,YAAA;EACA,eAAA;EACA,cAAA;EACA,iBAAA;ACzqCA;AD6qCA;EACA,aAAA;AC1qCA;AD6qCA;EACA,kBAAA;EACA,SAAA;EACA,UAAA;EACA,wBAAA;EACA,oCAAA;EACA,yCAAA;EACA,kBAAA;EACA,yCAAA;EACA,YAAA;EACA,aAAA;EACA,aAAA;EACA,sBAAA;EACA,QAAA;AC1qCA;AD6qCA;EACA,WAAA;EACA,gBAAA;EACA,sBAAA;EACA,kBAAA;EACA,eAAA;EACA,sBAAA;AC1qCA;AD4qCA;EACA,aAAA;EACA,qBAAA;AC1qCA;AD8qCA;EACA,WAAA;EACA,kBAAA;EACA,eAAA;EACA,0CAAA;AC3qCA;AD6qCA;EACA,iBAAA;EACA,gBAAA;AC3qCA;AD6qCA;EACA,gBAAA;EACA,uBAAA;AC3qCA;AD6qCA;EACA,uEAAA;AC3qCA;AD8qCA;EACA,cAAA;EACA,kBAAA;AC5qCA;ADirCA;EACA,aAAA;EACA,qBAAA;AC/qCA;ADmrCA;EACA,kBAAA;EACA,SAAA;EACA,WAAA;EACA,WAAA;EACA,gBAAA;EACA,YAAA;EACA,cAAA;EACA,eAAA;EACA,YAAA;EACA,eAAA;EACA,cAAA;EACA,kBAAA;AChrCA;ADkrCA;EACA,cAAA;EACA,yBAAA;AChrCA;ADorCA;EACA,aAAA;EACA,sBAAA;EACA,QAAA;EACA,WAAA;ACjrCA;ADorCA;EACA,aAAA;EACA,QAAA;EACA,mBAAA;ACjrCA;ADsrCA;EACA,eAAA;EACA,gBAAA;EACA,yCAAA;EACA,kBAAA;EACA,gEAAA;EACA,uBAAA;ACnrCA;ADqrCA;EACA,YAAA;ACnrCA;ADsrCA;EACA,WAAA;ACprCA;ADwrCA;EACA,aAAA;EACA,mBAAA;EACA,8BAAA;EACA,mBAAA;ACrrCA;ADurCA;EACA;qBAAA;ACprCA;ADyrCA;EACA,aAAA;EACA,YAAA;EACA,kBAAA;EACA,yCAAA;EACA,mBAAA;ACtrCA;ADyrCA;EACA,OAAA;EACA,gBAAA;EACA,eAAA;EACA,YAAA;EACA,uBAAA;EACA,eAAA;EACA,kBAAA;EACA,uBAAA;EACA,yBAAA;ACtrCA;ADwrCA;EACA,iEAAA;EACA,uBAAA;EACA,gBAAA;ACtrCA;ADyrCA;EACA,gEAAA;ACvrCA;AD2rCA;EACA,sBAAA;EACA,yCAAA;EACA,kBAAA;EACA,gBAAA;ACxrCA;AD2rCA;EACA,iBAAA;EACA,gBAAA;ACxrCA;AD2rCA;EACA,gBAAA;EACA,MAAA;EACA,gEAAA;EACA,gDAAA;EACA,aAAA;EACA,UAAA;ACxrCA;AD2rCA;EACA,mBAAA;ACxrCA;AD2rCA;EACA,WAAA;EACA,iBAAA;EACA,sBAAA;EACA,kBAAA;EACA,eAAA;EACA,mBAAA;ACxrCA;AD0rCA;EACA,aAAA;EACA,qBAAA;EACA,gBAAA;ACxrCA;AD2rCA;EACA,WAAA;ACzrCA;AD6rCA;EACA,aAAA;EACA,QAAA;AC1rCA;AD6rCA;EACA,OAAA;EACA,gBAAA;EACA,eAAA;EACA,yCAAA;EACA,kBAAA;EACA,gEAAA;EACA,eAAA;EACA,uBAAA;EACA,yBAAA;AC1rCA;AD4rCA;EACA,iEAAA;AC1rCA;AD8rCA;EACA,iBAAA;AC3rCA;AD8rCA;EACA,aAAA;EACA,mBAAA;EACA,gBAAA;EACA,eAAA;EACA,kBAAA;EACA,sCAAA;AC3rCA;AD6rCA;EACA,iEAAA;AC3rCA;AD+rCA;EACA,aAAA;EACA,mBAAA;EACA,uBAAA;EACA,WAAA;EACA,YAAA;EACA,kBAAA;AC5rCA;AD8rCA;EACA,WAAA;EACA,YAAA;EACA,SAAA;EACA,eAAA;AC5rCA;ADgsCA;EACA,OAAA;EACA,eAAA;EACA,gBAAA;EACA,uBAAA;EACA,mBAAA;AC7rCA;ADksCA;EACA,UAAA;AC/rCA;ADksCA;EACA,mBAAA;EACA,kBAAA;AChsCA;ADmsCA;EACA,gBAAA;EACA,kBAAA;ACjsCA;ADmsCA;EACA,gBAAA;ACjsCA;ADwsCA;EACA,WAAA;EACA,uBAAA;EACA,4BAAA;EACA,kBAAA;EACA,eAAA;EACA,kBAAA;EACA,sBAAA;EACA,sBAAA;ACrsCA;ADusCA;EACA,aAAA;EACA,qBAAA;ACrsCA;ADysCA;EACA,iBAAA;ACvsCA;AD2sCA;EACA,kBAAA;EACA,QAAA;EACA,UAAA;EACA,gBAAA;EACA,YAAA;EACA,WAAA;EACA,eAAA;EACA,eAAA;EACA,gBAAA;EACA,kBAAA;ACxsCA;AD0sCA;EACA,WAAA;EACA,mBAAA;ACxsCA;AD4sCA;EACA,eAAA;EACA,WAAA;ACzsCA;AD4sCA;EACA,WAAA;EACA,YAAA;EACA,sBAAA;EACA,kBAAA;EACA,eAAA;ACzsCA;AD2sCA;EACA,aAAA;EACA,qBAAA;ACzsCA;AD4sCA;EACA,qBAAA;EACA,yBAAA;AC1sCA;AD8sCA;EACA,cAAA;EACA,eAAA;EACA,eAAA;EACA,YAAA;EACA,yBAAA;EACA,kBAAA;AC3sCA;AD8sCA;EACA,gBAAA;EACA,SAAA;EACA,OAAA;EACA,QAAA;EACA,aAAA;EACA,oCAAA;EACA,6CAAA;EACA,aAAA;EACA,uBAAA;EACA,wBAAA;EACA,0BAAA;EACA,cAAA;EACA,WAAA;AC3sCA;AD6sCA;EACA,WAAA;EACA,YAAA;EACA,oCAAA;EACA,yCAAA;EACA,kBAAA;EACA,uBAAA;EACA,eAAA;EACA,eAAA;EACA,gBAAA;AC3sCA;AD6sCA;EACA,gEAAA;AC3sCA;;AAEA,qDAAqD","file":"st-pivottable-controls.vue","sourcesContent":["<template>\n  <div class=\"fields-container\">\n    <div class=\"zones-container\">\n      <!-- Rows Zone -->\n      <div \n        class=\"drop-zone section\"\n        data-zone=\"rows\"\n        :class=\"{ 'drag-over': dragOverZone === 'rows' }\"\n        @dragover.prevent=\"onDragOver($event, 'rows')\"\n        @dragleave=\"dragOverZone = null\"\n        @drop=\"onDrop($event, 'rows')\"\n      >\n        <div class=\"section-header\" @click=\"toggleSection('rows')\">\n          <h4>Rows</h4>\n          <div class=\"header-controls\">\n            <button \n              class=\"add-btn\"\n              @click.stop=\"showFieldSelect('rows')\"\n              title=\"Add Row\"\n            >\n              +\n            </button>\n            <span class=\"toggle-icon\" :class=\"{ 'collapsed': collapsedSections.rows }\">▼</span>\n          </div>\n        </div>\n        <div class=\"section-content\" :class=\"{ 'collapsed': collapsedSections.rows }\">\n          <div \n            v-for=\"field in config.rows\" \n            :key=\"field.field\"\n            class=\"field-item dropped\"\n            draggable=\"true\"\n            @dragstart=\"onDragStart($event, field.field, 'rows')\"\n            @dragend=\"onDragEnd\"\n          >\n            <div class=\"field-content\">\n              <span>{{ field.field }}</span>\n              <div class=\"field-controls\">\n                <select v-model=\"field.sortBy\" @change=\"onSortChange(field)\" class=\"sort-select\">\n                  <option value=\"label\">Label</option>\n                  <option v-for=\"value in config.values\" :key=\"value.id || `${value.field}_${value.aggregation}`\" :value=\"value.id || `${value.field}_${value.aggregation}`\">\n                    {{ value.field }} ({{ value.aggregation }})\n                  </option>\n                </select>\n                <select v-model=\"field.sortOrder\" @change=\"onSortChange(field)\" class=\"order-select\">\n                  <option value=\"asc\">Ascending</option>\n                  <option value=\"desc\">Descending</option>\n                </select>\n                <button class=\"remove-btn\" @click=\"removeField('rows', field)\">×</button>\n              </div>\n            </div>\n          </div>\n\n          <!-- Field Select Dropdown -->\n          <div v-if=\"activeAddField === 'rows'\" class=\"field-select-container\">\n            <input \n              type=\"text\" \n              v-model=\"searchQuery\" \n              class=\"field-search\"\n              placeholder=\"Search fields...\"\n              @input=\"filterFields\"\n              ref=\"searchInput\"\n              autofocus\n            />\n            <select \n              ref=\"rowsSelect\"\n              v-model=\"selectedField\" \n              @change=\"addNewField('rows')\" \n              class=\"field-select expanded\"\n              size=\"6\"\n            >\n              <option value=\"\" disabled>Select a field...</option>\n              <option \n                v-for=\"(field, index) in filteredFields\" \n                :key=\"`${field}_${index}`\"\n                :value=\"field\"\n              >\n                {{ field }}\n              </option>\n            </select>\n            <button class=\"cancel-btn\" @click=\"activeAddField = null\">×</button>\n          </div>\n        </div>\n      </div>\n\n      <!-- Columns Zone -->\n      <div \n        class=\"drop-zone section\"\n        data-zone=\"columns\"\n        :class=\"{ 'drag-over': dragOverZone === 'columns' }\"\n        @dragover.prevent=\"onDragOver($event, 'columns')\"\n        @dragleave=\"dragOverZone = null\"\n        @drop=\"onDrop($event, 'columns')\"\n      >\n        <div class=\"section-header\" @click=\"toggleSection('columns')\">\n          <h4>Columns</h4>\n          <div class=\"header-controls\">\n            <button \n              class=\"add-btn\"\n              @click.stop=\"showFieldSelect('columns')\"\n              title=\"Add Column\"\n            >\n              +\n            </button>\n            <span class=\"toggle-icon\" :class=\"{ 'collapsed': collapsedSections.columns }\">▼</span>\n          </div>\n        </div>\n        <div class=\"section-content\" :class=\"{ 'collapsed': collapsedSections.columns }\">\n          <div \n            v-for=\"field in config.columns\" \n            :key=\"field.field\"\n            class=\"field-item dropped\"\n            draggable=\"true\"\n            @dragstart=\"onDragStart($event, field.field, 'columns')\"\n            @dragend=\"onDragEnd\"\n          >\n            <div class=\"field-content\">\n              <span>{{ field.field }}</span>\n              <div class=\"field-controls\">\n                <select v-model=\"field.sortBy\" @change=\"onSortChange(field)\" class=\"sort-select\">\n                  <option value=\"label\">Label</option>\n                  <option v-for=\"value in config.values\" :key=\"value.id || `${value.field}_${value.aggregation}`\" :value=\"value.id || `${value.field}_${value.aggregation}`\">\n                    {{ value.field }} ({{ value.aggregation }})\n                  </option>\n                </select>\n                <select v-model=\"field.sortOrder\" @change=\"onSortChange(field)\" class=\"order-select\">\n                  <option value=\"asc\">Ascending</option>\n                  <option value=\"desc\">Descending</option>\n                </select>\n                <button class=\"remove-btn\" @click=\"removeField('columns', field)\">×</button>\n              </div>\n            </div>\n          </div>\n\n          <!-- Field Select Dropdown -->\n          <div v-if=\"activeAddField === 'columns'\" class=\"field-select-container\">\n            <input \n              type=\"text\" \n              v-model=\"searchQuery\" \n              class=\"field-search\"\n              placeholder=\"Search fields...\"\n              @input=\"filterFields\"\n              ref=\"searchInput\"\n              autofocus\n            />\n            <select \n              ref=\"columnsSelect\"\n              v-model=\"selectedField\" \n              @change=\"addNewField('columns')\" \n              class=\"field-select expanded\"\n              size=\"6\"\n            >\n              <option value=\"\" disabled>Select a field...</option>\n              <option \n                v-for=\"(field, index) in filteredFields\" \n                :key=\"`${field}_${index}`\"\n                :value=\"field\"\n              >\n                {{ field }}\n              </option>\n            </select>\n            <button class=\"cancel-btn\" @click=\"activeAddField = null\">×</button>\n          </div>\n        </div>\n      </div>\n\n      <!-- Values Zone -->\n      <div \n        class=\"drop-zone section\"\n        data-zone=\"values\"\n        :class=\"{ 'drag-over': dragOverZone === 'values' }\"\n        @dragover.prevent=\"onDragOver($event, 'values')\"\n        @dragleave=\"dragOverZone = null\"\n        @drop=\"onDrop($event, 'values')\"\n      >\n        <div class=\"section-header\" @click=\"toggleSection('values')\">\n          <h4>Values</h4>\n          <div class=\"header-controls\">\n            <button \n              class=\"add-btn\"\n              @click.stop=\"showFieldSelect('values')\"\n              title=\"Add Value\"\n            >\n              +\n            </button>\n            <span class=\"toggle-icon\" :class=\"{ 'collapsed': collapsedSections.values }\">▼</span>\n          </div>\n        </div>\n        <div class=\"section-content\" :class=\"{ 'collapsed': collapsedSections.values }\">\n          <div \n            v-for=\"field in config.values\" \n            :key=\"field.id\"\n            class=\"field-item dropped\"\n            draggable=\"true\"\n            @dragstart=\"onDragStart($event, field.field, 'values')\"\n            @dragend=\"onDragEnd\"\n          >\n            <div class=\"field-content\">\n              {{ field.field }}\n              <select v-model=\"field.aggregation\" @change=\"onAggregationChange(field)\">\n                <option value=\"sum\">SUM</option>\n                <option value=\"counta\">COUNTA</option>\n                <option value=\"count\">COUNT</option>\n                <option value=\"countunique\">COUNTUNIQUE</option>\n                <option value=\"average\">AVERAGE</option>\n                <option value=\"max\">MAX</option>\n                <option value=\"min\">MIN</option>\n                <option value=\"median\">MEDIAN</option>\n                <option value=\"stdev\">STDEV</option>\n                <option value=\"stdevp\">STDEVP</option>\n                <option value=\"var\">VAR</option>\n                <option value=\"varp\">VARP</option>\n                <option value=\"custom\">Custom Formula</option>\n              </select>\n              <div v-if=\"field.aggregation === 'custom'\" class=\"formula-input-container\">\n                <input \n                  type=\"text\" \n                  v-model=\"field.formula\" \n                  class=\"formula-input\"\n                  placeholder=\"Enter formula (e.g. {Revenue} - {Costs})\"\n                  @input=\"validateAndUpdateFormula(field)\"\n                  :class=\"{ 'error': field.formulaError }\"\n                />\n                <div v-if=\"field.formulaError\" class=\"formula-error\">\n                  {{ field.formulaError }}\n                </div>\n              </div>\n            </div>\n            <button class=\"remove-btn\" @click=\"removeField('values', field)\">×</button>\n          </div>\n\n          <!-- Field Select Dropdown -->\n          <div v-if=\"activeAddField === 'values'\" class=\"field-select-container\">\n            <input \n              type=\"text\" \n              v-model=\"searchQuery\" \n              class=\"field-search\"\n              placeholder=\"Search fields...\"\n              @input=\"filterFields\"\n              ref=\"searchInput\"\n              autofocus\n            />\n            <select \n              ref=\"valuesSelect\"\n              v-model=\"selectedField\" \n              @change=\"addNewField('values')\" \n              class=\"field-select expanded\"\n              size=\"6\"\n            >\n              <option value=\"\" disabled>Select a field...</option>\n              <option \n                v-for=\"(field, index) in filteredFields\" \n                :key=\"`${field}_${index}`\"\n                :value=\"field\"\n              >\n                {{ field }}\n              </option>\n            </select>\n            <button class=\"cancel-btn\" @click=\"activeAddField = null\">×</button>\n          </div>\n        </div>\n      </div>\n\n      <!-- Filters Zone -->\n      <div \n        class=\"drop-zone section\"\n        data-zone=\"filters\"\n        :class=\"{ \n          'drag-over': dragOverZone === 'filters',\n          'has-no-results': hasNoResults && filters.length > 0 \n        }\"\n        @dragover.prevent=\"onDragOver($event, 'filters')\"\n        @dragleave=\"dragOverZone = null\"\n        @drop=\"onDrop($event, 'filters')\"\n      >\n        <div class=\"section-header\" @click=\"toggleSection('filters')\">\n          <h4>Filters</h4>\n          <div class=\"header-controls\">\n            <button \n              class=\"add-btn\"\n              @click.stop=\"showFilterSelect\"\n              title=\"Add Filter\"\n            >\n              +\n            </button>\n            <span class=\"toggle-icon\" :class=\"{ 'collapsed': collapsedSections.filters }\">▼</span>\n          </div>\n        </div>\n        <div class=\"section-content\" :class=\"{ 'collapsed': collapsedSections.filters }\">\n          <div \n            v-for=\"(filter, index) in filters\" \n            :key=\"`filter_${filter.column}_${index}`\"\n            class=\"field-item dropped\"\n            draggable=\"true\"\n            @dragstart=\"onDragStart($event, filter.column, 'filters')\"\n            @dragend=\"onDragEnd\">\n            <div class=\"field-content\">\n              <div class=\"field-header\">\n                <span class=\"field-name\">{{ filter.column }}</span>\n              </div>\n              <div class=\"filter-type-selector\">\n                <button class=\"filter-type-btn\" \n                        :class=\"{ active: filter.filterType === 'condition' }\"\n                        @click=\"toggleFilterType(filter)\">\n                  Condition\n                </button>\n                <button class=\"filter-type-btn\" \n                        :class=\"{ active: filter.filterType === 'values' }\"\n                        @click=\"toggleFilterType(filter)\">\n                  Values\n                </button>\n              </div>\n\n              <!-- Values-based filter controls -->\n              <div v-if=\"filter.filterType === 'values'\" class=\"filter-controls values-filter\">\n                <div class=\"values-list\">\n                  <div class=\"values-list-header\">\n                    <div class=\"search-container\">\n                      <input type=\"text\" \n                             v-model=\"filter.searchQuery\"\n                             placeholder=\"Search values...\"\n                             class=\"values-search\">\n                    </div>\n                    <div class=\"values-actions\">\n                      <button class=\"action-btn\" @click=\"selectAllValues(filter)\">Select All</button>\n                      <button class=\"action-btn\" @click=\"clearValues(filter)\">Clear</button>\n                    </div>\n                  </div>\n                  <div class=\"values-options\">\n                    <label v-for=\"value in filteredValues(filter)\" \n                           :key=\"String(value)\"\n                           class=\"value-option\">\n                      <div class=\"checkbox-wrapper\">\n                        <input type=\"checkbox\" \n                               :value=\"value\"\n                               v-model=\"filter.selectedValues\"\n                               @change=\"filterChanged\">\n                      </div>\n                      <span class=\"value-label\">{{ formatValue(value) }}</span>\n                    </label>\n                  </div>\n                </div>\n              </div>\n\n              <!-- Condition-based filter controls -->\n              <div v-else class=\"filter-controls condition-filter\">\n                <select v-model=\"filter.condition\" @change=\"filterChanged\">\n                  <option value=\"equals\">Equals</option>\n                  <option value=\"notEquals\">Does not equal</option>\n                  <option value=\"greaterThan\">Greater than</option>\n                  <option value=\"greaterThanOrEqual\">Greater than or equal</option>\n                  <option value=\"lessThan\">Less than</option>\n                  <option value=\"lessThanOrEqual\">Less than or equal</option>\n                  <option value=\"contains\">Contains</option>\n                  <option value=\"notContains\">Does not contain</option>\n                  <option value=\"startsWith\">Starts with</option>\n                  <option value=\"endsWith\">Ends with</option>\n                  <option value=\"isEmpty\">Is empty</option>\n                  <option value=\"isNotEmpty\">Is not empty</option>\n                </select>\n                \n                <template v-if=\"!['isEmpty', 'isNotEmpty'].includes(filter.condition)\">\n                  <input v-if=\"getInputType(filter.column) === 'checkbox'\"\n                         type=\"checkbox\"\n                         v-model=\"filter.value\"\n                         @change=\"filterChanged\">\n                  <input v-else-if=\"getInputType(filter.column) === 'radio'\"\n                         type=\"radio\"\n                         v-model=\"filter.value\"\n                         @change=\"filterChanged\">\n                  <input v-else\n                         :type=\"getInputType(filter.column)\"\n                         v-model=\"filter.value\"\n                         placeholder=\"Value\"\n                         @change=\"filterChanged\">\n                </template>\n              </div>\n            </div>\n            <button class=\"remove-btn\" @click=\"removeFilter(filter)\">×</button>\n          </div>\n\n          <!-- Field Select Dropdown -->\n          <div v-if=\"showColumnSelect\" class=\"field-select-container\">\n            <input \n              type=\"text\" \n              v-model=\"searchQuery\" \n              class=\"field-search\"\n              placeholder=\"Search fields...\"\n              @input=\"filterFields\"\n              ref=\"searchInput\"\n              autofocus\n            />\n            <select \n              ref=\"filterSelect\"\n              v-model=\"selectedColumn\" \n              @change=\"addNewFilter\" \n              class=\"field-select expanded\"\n              size=\"6\"\n            >\n              <option value=\"\" disabled>Select a field...</option>\n              <option \n                v-for=\"(field, index) in filteredColumns\" \n                :key=\"`${field}_${index}`\"\n                :value=\"field\"\n              >\n                {{ field }}\n              </option>\n            </select>\n            <button class=\"cancel-btn\" @click=\"showColumnSelect = false\">×</button>\n          </div>\n        </div>\n      </div>\n    </div>\n    <div class=\"actions-panel\">\n      <button class=\"export-btn\" @click=\"exportData\" title=\"Export CSV\">\n        Export CSV\n      </button>\n    </div>\n  </div>\n</template>\n\n<script>\nexport default {\n  name: 'st-pivottable-controls',\n  \n  props: {\n    data: {\n      type: Array,\n      required: true\n    },\n    rows: {\n      type: Array,\n      default: () => []\n    },\n    columns: {\n      type: Array,\n      default: () => []\n    },\n    values: {\n      type: Array,\n      default: () => []\n    },\n    filters: {\n      type: Array,\n      default: () => []\n    },\n    hasNoResults: {\n      type: Boolean,\n      default: false\n    }\n  },\n\n  data() {\n    return {\n      dragOverZone: null,\n      isDragging: false,\n      dragSourceZone: null,\n      collapsedSections: {\n        available: false,\n        rows: false,\n        columns: false,\n        values: false,\n        filters: false\n      },\n      showColumnSelect: false,\n      selectedColumn: '',\n      activeAddField: null,\n      selectedField: '',\n      searchQuery: '',\n      filteredFields: [],\n      nextValueId: 1,  // Counter for generating unique IDs\n      config: {\n        rows: this.rows.map(row => ({\n          ...row,\n          sortBy: row.sortBy || 'label',\n          sortOrder: row.sortOrder || 'asc'\n        })),\n        columns: this.columns || [],\n        values: (this.values || []).map(value => ({\n          ...value,\n          id: value.id || `${value.field}_${value.aggregation}_${this.nextValueId++}`\n        }))\n      },\n      uniqueValuesCache: {},\n      isUpdatingConfig: false  // Add this flag to prevent recursive updates\n    }\n  },\n\n  computed: {\n    availableFields() {\n      console.log('Computing availableFields');\n      const fields = Object.keys(this.data[0] || {}).sort((a, b) => a.localeCompare(b));\n      console.log('Available fields:', fields);\n      return fields;\n    },\n\n    restrictedFields() {\n      console.log('Computing restrictedFields');\n      const usedFields = new Set([\n        ...this.config.rows.map(f => f.field),\n        ...this.config.columns.map(f => f.field)\n      ]);\n      console.log('Used fields:', Array.from(usedFields));\n      const fields = this.availableFields.filter(field => !usedFields.has(field));\n      console.log('Restricted fields:', fields);\n      return fields;\n    },\n\n    availableColumns() {\n      console.log('Computing availableColumns');\n      const fields = Object.keys(this.data[0] || {}).sort((a, b) => a.localeCompare(b));\n      console.log('Available columns:', fields);\n      return fields;\n    },\n\n    filteredColumns() {\n      console.log('Computing filteredColumns');\n      const query = this.searchQuery.toLowerCase();\n      const fields = this.availableColumns\n        .filter(field => field.toLowerCase().includes(query))\n        .sort((a, b) => a.localeCompare(b));\n      console.log('Filtered columns:', fields);\n      return fields;\n    }\n  },\n\n  methods: {\n    onDragStart(event, field, sourceZone = null) {\n      this.isDragging = true;\n      this.dragSourceZone = sourceZone;\n      \n      // Set data for transfer\n      event.dataTransfer.setData('text/plain', field);\n      event.dataTransfer.setData('source-zone', sourceZone || '');\n      \n      // If it's a Values or Filters item, only allow reordering within their own pod\n      if (sourceZone === 'values' || sourceZone === 'filters') {\n        event.dataTransfer.effectAllowed = 'copyMove';\n      }\n\n      // If dragging from within a zone, disable pointer events on other fields in that zone\n      if (sourceZone) {\n        const dropZone = event.target.closest('.drop-zone');\n        const fields = dropZone.querySelectorAll('.field-item');\n        fields.forEach(fieldEl => {\n          if (fieldEl !== event.target) {\n            fieldEl.style.pointerEvents = 'none';\n          }\n        });\n      }\n    },\n\n    onDragEnd() {\n      // Re-enable pointer events on all fields\n      document.querySelectorAll('.field-item').forEach(fieldEl => {\n        fieldEl.style.pointerEvents = '';\n      });\n      this.isDragging = false;\n      this.dragSourceZone = null;\n    },\n\n    onDragOver(event, zone) {\n      const sourceZone = this.dragSourceZone;\n      \n      // Prevent dropping Values items into other zones\n      if (sourceZone === 'values' && zone !== 'values') {\n        event.preventDefault();\n        return;\n      }\n      \n      // Prevent dropping Filters items into other zones\n      if (sourceZone === 'filters' && zone !== 'filters') {\n        event.preventDefault();\n        return;\n      }\n      \n      // Prevent dropping other items into Values or Filters zones\n      if ((sourceZone !== 'values' && zone === 'values') || \n          (sourceZone !== 'filters' && zone === 'filters')) {\n        event.preventDefault();\n        return;\n      }\n      \n      event.preventDefault();\n      this.dragOverZone = zone;\n      \n      const dropZone = event.currentTarget;\n      const fieldItems = Array.from(dropZone.querySelectorAll('.field-item'));\n      \n      if (fieldItems.length === 0) {\n        // If no items, show indicator at the top\n        this.dropPosition = 'top';\n        this.dropIndicatorStyle = { top: '40px' };\n        return;\n      }\n\n      // Find the insertion point\n      let insertBefore = null;\n      let insertIndex = 0;\n      for (let i = 0; i < fieldItems.length; i++) {\n        const rect = fieldItems[i].getBoundingClientRect();\n        const midY = rect.top + rect.height / 2;\n        \n        if (event.clientY < midY) {\n          insertBefore = fieldItems[i];\n          insertIndex = i;\n          break;\n        }\n      }\n\n      if (insertBefore) {\n        // Position indicator above this item\n        const rect = insertBefore.getBoundingClientRect();\n        const dropZoneRect = dropZone.getBoundingClientRect();\n        const relativeTop = rect.top - dropZoneRect.top;\n        this.dropIndicatorStyle = { top: `${relativeTop}px` };\n        this.dropPosition = insertIndex === 0 ? 'top' : null;\n      } else {\n        // Position indicator at the bottom\n        this.dropPosition = 'bottom';\n        this.dropIndicatorStyle = null;\n      }\n    },\n\n    onDrop(event, targetZone) {\n      event.preventDefault();\n      this.dragOverZone = null;\n      \n      const field = event.dataTransfer.getData('text/plain');\n      const sourceZone = event.dataTransfer.getData('source-zone');\n      \n      // Prevent dropping Values items into other zones\n      if (sourceZone === 'values' && targetZone !== 'values') {\n        return;\n      }\n      \n      // Prevent dropping Filters items into other zones\n      if (sourceZone === 'filters' && targetZone !== 'filters') {\n        return;\n      }\n      \n      // Prevent dropping other items into Values or Filters zones\n      if ((sourceZone !== 'values' && targetZone === 'values') || \n          (sourceZone !== 'filters' && targetZone === 'filters')) {\n        return;\n      }\n      \n      // Remove any drag-over classes\n      const fieldItems = Array.from(event.currentTarget.querySelectorAll('.field-item'));\n      fieldItems.forEach(item => item.classList.remove('drag-over-top', 'drag-over-bottom'));\n      \n      // Re-enable pointer events on all fields\n      document.querySelectorAll('.field-item').forEach(fieldEl => {\n        fieldEl.style.pointerEvents = '';\n      });\n      \n      // If source and target zones are the same, handle reordering\n      if (sourceZone === targetZone) {\n        let items;\n        let draggedItem;\n        \n        if (targetZone === 'filters') {\n          items = [...this.filters];\n          draggedItem = items.find(f => f.column === field);\n        } else {\n          items = [...this.config[targetZone]];\n          draggedItem = items.find(f => f.field === field);\n        }\n        \n        if (!draggedItem) return;\n\n        let insertIndex = items.length;\n        \n        for (let i = 0; i < fieldItems.length; i++) {\n          const item = fieldItems[i];\n          const box = item.getBoundingClientRect();\n          const itemMiddle = box.top + box.height / 2;\n          \n          if (event.clientY < itemMiddle) {\n            insertIndex = i;\n            break;\n          }\n        }\n        \n        // Remove the dragged item from its current position\n        const draggedIndex = items.indexOf(draggedItem);\n        items.splice(draggedIndex, 1);\n        items.splice(insertIndex, 0, draggedItem);\n        \n        // Update the appropriate array\n        if (targetZone === 'filters') {\n          this.$emit('filtersChanged', items);\n        } else {\n          this.isUpdatingConfig = true;\n          try {\n            this.config[targetZone] = items;\n            this.$emit('configChanged', { ...this.config });\n          } finally {\n            this.isUpdatingConfig = false;\n          }\n        }\n      } else if (targetZone !== 'values' && targetZone !== 'filters') {  // Only handle cross-pod drops for non-Values/non-Filters zones\n        // Handle dropping between different zones (existing cross-pod logic)\n        let draggedField;\n\n        // Find the field in the source zone if it exists\n        if (sourceZone && this.config[sourceZone]) {\n          draggedField = this.config[sourceZone].find(f => f.field === field);\n        }\n\n        // If no dragged field found (from available fields) create a new one\n        if (!draggedField) {\n          draggedField = { field };\n        }\n\n        // Add default sort properties for rows\n        if (targetZone === 'rows') {\n          draggedField = {\n            ...draggedField,\n            sortBy: 'label',\n            sortOrder: 'asc'\n          };\n        }\n\n        // Remove from source zone if it exists\n        if (sourceZone && this.config[sourceZone]) {\n          const index = this.config[sourceZone].findIndex(f => f.field === field);\n          if (index !== -1) {\n            const newSourceFields = [...this.config[sourceZone]];\n            newSourceFields.splice(index, 1);\n            this.config[sourceZone] = newSourceFields;\n          }\n        }\n\n        // Add to target zone\n        this.isUpdatingConfig = true;\n        try {\n          this.config[targetZone] = [...this.config[targetZone], draggedField];\n          this.$emit('configChanged', { ...this.config });\n        } finally {\n          this.isUpdatingConfig = false;\n        }\n      }\n      \n      this.isDragging = false;\n      this.dragSourceZone = null;\n      this.dropPosition = null;\n      this.dropIndicatorStyle = null;\n    },\n\n    removeField(zone, field) {\n      console.log('Removing field:', { zone, field });\n      const index = this.config[zone].findIndex(f => f.field === field.field);\n      if (index !== -1) {\n        // Create a new array instead of modifying the existing one\n        this.config[zone] = [\n          ...this.config[zone].slice(0, index),\n          ...this.config[zone].slice(index + 1)\n        ];\n        console.log('New config after removal:', this.config[zone]);\n        this.$emit('configChanged', { ...this.config });\n      }\n    },\n\n    toggleSection(section) {\n      this.collapsedSections[section] = !this.collapsedSections[section]\n    },\n\n    removeFilter(filter) {\n      const index = this.filters.indexOf(filter);\n      if (index !== -1) {\n        this.filters.splice(index, 1);\n        this.$emit('filtersChanged', this.filters);\n      }\n    },\n\n    filterChanged() {\n      this.$emit('filtersChanged', this.filters);\n    },\n\n    getInputType(column) {\n      // Determine input type based on the first non-null value in the column\n      const firstValue = this.data.find(row => row[column] != null)?.[column];\n      if (typeof firstValue === 'number') return 'number';\n      if (typeof firstValue === 'boolean') return 'checkbox';\n      if (firstValue instanceof Date) return 'date';\n      return 'text';\n    },\n\n    getDefaultCondition(column) {\n      const firstValue = this.data.find(row => row[column] != null)?.[column];\n      \n      if (typeof firstValue === 'number') return 'equals';\n      if (typeof firstValue === 'boolean') return 'equals';\n      if (firstValue instanceof Date) return 'equals';\n      return 'contains';\n    },\n\n    getDefaultValue(column) {\n      const firstValue = this.data.find(row => row[column] != null)?.[column];\n      \n      if (typeof firstValue === 'number') return 0;\n      if (typeof firstValue === 'boolean') return true;\n      if (firstValue instanceof Date) return new Date().toISOString().split('T')[0];\n      return '';\n    },\n\n    addNewFilter() {\n      if (!this.selectedColumn) return;\n\n      const newFilter = {\n        column: this.selectedColumn,\n        filterType: 'condition',\n        condition: this.getDefaultCondition(this.selectedColumn),\n        value: this.getDefaultValue(this.selectedColumn),\n        selectedValues: [],\n        searchQuery: ''\n      };\n\n      // Use Vue.set to ensure reactivity\n      if (this.filters.push) {\n        this.filters.push(newFilter);\n      } else {\n        this.$set(this, 'filters', [...(this.filters || []), newFilter]);\n      }\n      \n      this.$emit('filtersChanged', this.filters);\n      \n      // Reset selection\n      this.selectedColumn = '';\n      this.showColumnSelect = false;\n    },\n\n    addNewField(zone) {\n      console.log('Adding new field:', { zone, selectedField: this.selectedField });\n      if (!this.selectedField) return;\n      \n      let newField = { field: this.selectedField };\n\n      // Add zone-specific properties\n      switch (zone) {\n        case 'rows': {\n          newField = {\n            ...newField,\n            sortBy: 'label',\n            sortOrder: 'asc'\n          };\n          break;\n        }\n        case 'values': {\n          // Determine best default aggregation based on field type\n          const firstValue = this.data.find(row => row[this.selectedField] != null)?.[this.selectedField];\n          const defaultAggregation = typeof firstValue === 'number' ? 'sum' : 'count';\n          const newId = `${this.selectedField}_${defaultAggregation}_${this.nextValueId++}`;\n          console.log('Creating new value field:', { field: this.selectedField, aggregation: defaultAggregation, id: newId });\n          \n          newField = {\n            ...newField,\n            aggregation: defaultAggregation,\n            id: newId\n          };\n          break;\n        }\n      }\n      \n      // Create a new array instead of pushing to the existing one\n      this.config[zone] = [...this.config[zone], newField];\n      console.log('New config after addition:', this.config[zone]);\n      \n      // Emit the change with a new config object\n      this.$emit('configChanged', { ...this.config });\n      \n      this.selectedField = '';\n      this.activeAddField = null;\n    },\n\n    onSortChange() {\n      console.log('Sort changed, emitting new config');\n      this.$emit('configChanged', { ...this.config });\n    },\n\n    showFieldSelect(zone) {\n      this.activeAddField = zone;\n      this.searchQuery = '';\n      // Initialize with appropriate field list based on zone\n      this.filteredFields = (zone === 'rows' || zone === 'columns') \n        ? this.restrictedFields \n        : this.availableFields;\n      this.$nextTick(() => {\n        const searchInput = this.$refs.searchInput;\n        if (searchInput) {\n          searchInput.focus();\n        }\n      });\n    },\n\n    showFilterSelect() {\n      this.showColumnSelect = true;\n      this.searchQuery = '';\n      // Show all fields for filters\n      this.filteredFields = this.availableFields;\n      this.$nextTick(() => {\n        const searchInput = this.$refs.searchInput;\n        if (searchInput) {\n          searchInput.focus();\n        }\n      });\n    },\n\n    filterFields() {\n      console.log('Filtering fields');\n      const query = this.searchQuery.toLowerCase();\n      const sourceFields = (this.activeAddField === 'rows' || this.activeAddField === 'columns')\n        ? this.restrictedFields\n        : this.availableFields;\n      \n      this.filteredFields = sourceFields\n        .filter(field => field.toLowerCase().includes(query))\n        .sort((a, b) => a.localeCompare(b));\n      console.log('Filtered fields:', this.filteredFields);\n    },\n\n    closeDropdown() {\n      if (this.activeAddField) {\n        this.activeAddField = null;\n      }\n      if (this.showColumnSelect) {\n        this.showColumnSelect = false;\n      }\n      this.selectedField = '';\n      this.selectedColumn = '';\n      this.searchQuery = '';\n    },\n\n    handleKeydown(event) {\n      if (event.key === 'Escape') {\n        this.closeDropdown();\n      }\n    },\n\n    onAggregationChange(field) {\n      if (field.aggregation === 'custom') {\n        // Initialize formula if not exists\n        if (!field.formula) {\n          field.formula = '';\n        }\n        // Validate empty formula\n        this.validateFormula(field);\n      } else {\n        // Clean up formula-related properties when switching back to standard aggregation\n        delete field.formula;\n        delete field.formulaError;\n      }\n      \n      // Update the field's ID to reflect the change\n      field.id = `${field.field}_${field.aggregation}_${this.nextValueId++}`;\n      \n      // Create a new config object to ensure proper reactivity\n      const newConfig = {\n        ...this.config,\n        values: this.config.values.map(v => v === field ? { ...field } : v)\n      };\n      \n      // Emit the change with the new config\n      this.$emit('configChanged', newConfig);\n    },\n\n    toggleFilterType(filter) {\n      filter.filterType = filter.filterType === 'condition' ? 'values' : 'condition';\n      if (filter.filterType === 'values') {\n        // Initialize all necessary properties for values filter type\n        this.$set(filter, 'selectedValues', []);\n        this.$set(filter, 'searchQuery', '');\n      } else {\n        filter.condition = this.getDefaultCondition(filter.column);\n        filter.value = '';\n      }\n      this.filterChanged();\n    },\n\n    getUniqueValues(column) {\n      if (!this.uniqueValuesCache[column]) {\n        const values = new Set();\n        this.data.forEach(row => {\n          if (row[column] !== null && row[column] !== undefined) {\n            values.add(row[column]);\n          }\n        });\n        this.uniqueValuesCache[column] = Array.from(values).sort((a, b) => {\n          return String(a).localeCompare(String(b));\n        });\n      }\n      return this.uniqueValuesCache[column];\n    },\n\n    selectAllValues(filter) {\n      filter.selectedValues = [...this.getUniqueValues(filter.column)];\n      this.filterChanged();\n    },\n\n    clearValues(filter) {\n      filter.selectedValues = [];\n      this.filterChanged();\n    },\n\n    filteredValues(filter) {\n      const values = this.getUniqueValues(filter.column);\n      if (!filter.searchQuery) return values;\n      \n      const searchTerm = (filter.searchQuery || '').toLowerCase();\n      return values.filter(value => \n        String(value).toLowerCase().includes(searchTerm)\n      );\n    },\n\n    formatValue(value) {\n      return typeof value === 'number' ? value.toLocaleString() : value;\n    },\n\n    validateAndUpdateFormula(field) {\n      console.log('FORMULA_UPDATE: Validating formula:', field.formula);\n      const isValid = this.validateFormula(field);\n      \n      if (isValid) {\n        console.log('FORMULA_UPDATE: Formula is valid, updating config');\n        // Create a new config object to ensure proper reactivity\n        const newConfig = {\n          ...this.config,\n          values: this.config.values.map(v => v === field ? { ...field } : v)\n        };\n        \n        // Emit the change with the new config\n        this.$emit('configChanged', newConfig);\n      }\n    },\n\n    validateFormula(field) {\n      // Reset error state\n      field.formulaError = null;\n      \n      if (!field.formula || !field.formula.trim()) {\n        field.formulaError = 'Formula cannot be empty';\n        return false;\n      }\n\n      // Extract field references from formula\n      const fieldRefs = field.formula.match(/\\{([^}]+)\\}/g) || [];\n      const functionCalls = field.formula.match(/\\b(SUM|COUNT|COUNTA|COUNTUNIQUE|AVERAGE|MAX|MIN|MEDIAN|STDEV|STDEVP|VAR|VARP)\\s*\\{([^}]+)\\}/g) || [];\n      \n      // Check if formula contains at least one field reference or function call\n      if (fieldRefs.length === 0 && functionCalls.length === 0) {\n        field.formulaError = 'Formula must reference at least one field using {fieldName} or a function call like SUM{fieldName}';\n        return false;\n      }\n\n      // Validate that all referenced fields exist in source data\n      const availableFields = Object.keys(this.data[0] || {});\n      const allFieldRefs = [...fieldRefs];\n      \n      // Extract field references from function calls\n      functionCalls.forEach(func => {\n        const match = func.match(/\\{([^}]+)\\}/);\n        if (match) {\n          allFieldRefs.push(match[0]);\n        }\n      });\n\n      // Validate all field references\n      for (const ref of new Set(allFieldRefs)) {  // Use Set to remove duplicates\n        const fieldName = ref.slice(1, -1); // Remove { }\n        if (!availableFields.includes(fieldName)) {\n          field.formulaError = `Field \"${fieldName}\" not found in source data`;\n          return false;\n        }\n      }\n\n      // Basic syntax validation (this can be enhanced later)\n      try {\n        // Replace field references and function calls with 1 for validation\n        let testFormula = field.formula;\n        \n        // First replace function calls\n        testFormula = testFormula.replace(/\\b(SUM|COUNT|COUNTA|COUNTUNIQUE|AVERAGE|MAX|MIN|MEDIAN|STDEV|STDEVP|VAR|VARP)\\s*\\{[^}]+\\}/g, '1');\n        \n        // Then replace remaining field references\n        testFormula = testFormula.replace(/\\{[^}]+\\}/g, '1');\n        \n        // Use Function instead of eval for safer evaluation\n        new Function(`return ${testFormula}`)();\n        return true;\n      } catch (error) {\n        field.formulaError = 'Invalid formula syntax';\n        return false;\n      }\n    },\n\n    exportData() {\n        this.$emit('export');\n    }\n  },\n\n  mounted() {\n    document.addEventListener('keydown', this.handleKeydown);\n  },\n\n  beforeDestroy() {\n    document.removeEventListener('keydown', this.handleKeydown);\n  },\n\n  watch: {\n    rows: {\n      handler(newRows) {\n        if (this.isUpdatingConfig) return;  // Skip if already updating\n        \n        this.isUpdatingConfig = true;\n        try {\n          this.config.rows = newRows.map(row => ({\n            ...row,\n            sortBy: row.sortBy || 'label',\n            sortOrder: row.sortOrder || 'asc'\n          }));\n        } finally {\n          this.isUpdatingConfig = false;\n        }\n      },\n      deep: true\n    },\n    columns: {\n      handler(newColumns) {\n        if (this.isUpdatingConfig) return;  // Skip if already updating\n        \n        this.isUpdatingConfig = true;\n        try {\n          this.config.columns = newColumns;\n        } finally {\n          this.isUpdatingConfig = false;\n        }\n      },\n      deep: true\n    },\n    values: {\n      handler(newValues) {\n        if (this.isUpdatingConfig) return;  // Skip if already updating\n        \n        this.isUpdatingConfig = true;\n        try {\n          this.config.values = newValues.map(value => ({\n            ...value,\n            id: value.id || `${value.field}_${value.aggregation}_${this.nextValueId++}`\n          }));\n        } finally {\n          this.isUpdatingConfig = false;\n        }\n      },\n      deep: true\n    }\n  }\n}\n</script>\n\n<style lang=\"scss\" scoped>\n.fields-container {\n  display: flex;\n  flex-direction: column;\n  gap: 16px;\n  width: 300px;\n  background-color: var(--st-dashboard-bg-0);\n  padding: 6px;\n  border-radius: 8px;\n  border: 1px solid var(--st-color-neutral);\n  height: 100%;\n  box-sizing: border-box;\n  position: relative;\n}\n\n.zones-container {\n  display: flex;\n  flex-direction: column;\n  gap: 2px;\n  flex: 1;\n  overflow-y: auto;\n  padding-bottom: 12px; // Add padding to prevent content from being hidden behind actions panel\n  \n  // Improve scrollbar styling\n  &::-webkit-scrollbar {\n    width: 8px;\n    height: 8px;\n  }\n\n  &::-webkit-scrollbar-track {\n    background: transparent;\n  }\n\n  &::-webkit-scrollbar-thumb {\n    background: var(--st-color-neutral);\n    border-radius: 4px;\n\n    &:hover {\n      background: color-mix(in srgb, var(--st-color-neutral) 80%, transparent);\n    }\n  }\n\n  &::-webkit-scrollbar-corner {\n    background: transparent;\n  }\n}\n\n.field-item {\n  position: relative;\n  z-index: 1;\n  padding: 12px 12px 12px 12px;\n  background: #f8f9fa;\n  border: 1px solid #e9ecef;\n  border-radius: 4px;\n  margin-bottom: 8px;\n  cursor: move;\n  display: flex;\n  justify-content: space-between;\n  align-items: flex-start;\n  font-size: 13px;\n  \n  &:last-child {\n    margin-bottom: 0;\n  }\n  \n  &.dropped {\n    background: color-mix(in srgb, var(--st-text-2) 5%, transparent);\n    border-color: var(--st-color-neutral);\n    color: var(--st-text-1);\n\n    // Add visual feedback when filters cause no results\n    .has-no-results & {\n      background: color-mix(in srgb, #ff0000 15%, transparent);\n      border-color: color-mix(in srgb, #ff0000 35%, transparent);\n\n      .filter-controls {\n        select, input {\n          border-color: color-mix(in srgb, #ff0000 35%, transparent);\n          \n          &:focus {\n            border-color: #ef5350;\n          }\n        }\n      }\n    }\n  }\n  \n  &.drag-over-top {\n    border-top: 2px solid #0066cc;\n  }\n  \n  &.drag-over-bottom {\n    border-bottom: 2px solid #0066cc;\n  }\n}\n\n.remove-btn {\n  position: absolute;\n  top: 2px;\n  right: 2px;\n  background: none;\n  border: none;\n  color: #adb5bd;\n  cursor: pointer;\n  padding: 4px;\n  font-size: 16px;\n  line-height: 1;\n  z-index: 2;\n  border-radius: 4px;\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  width: 20px;\n  height: 20px;\n  \n  &:hover {\n    color: #495057;\n    background-color: rgba(0, 0, 0, 0.05);\n  }\n}\n\n.field-content {\n  display: flex;\n  flex-direction: column;\n  gap: 6px;\n  flex: 1;\n  min-width: 0;\n}\n\n.section {\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 4px;\n  background: var(--st-dashboard-bg-1);\n\n  .section-header {\n    padding: 5px 10px;\n    background: var(--st-dashboard-bg-2);\n    border-bottom: 1px solid var(--st-color-neutral);\n    cursor: pointer;\n    display: flex;\n    justify-content: space-between;\n    align-items: center;\n    user-select: none;\n\n    &:hover {\n      background: color-mix(in srgb, var(--st-dashboard-bg-2), var(--st-text-1) 10%);\n    }\n\n    h4 {\n      margin: 0;\n      color: var(--st-text-1);\n      font-size: 14px;\n      font-weight: 600;\n    }\n  }\n\n  .section-content {\n    padding: 7px 5px;\n    transition: max-height 0.3s ease-in-out;\n    overflow: visible;\n    position: relative;\n\n    &.collapsed {\n      max-height: 0;\n      padding: 0;\n      overflow: hidden;\n    }\n  }\n}\n\n.header-controls {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n}\n\n.add-btn {\n  background: none;\n  border: none;\n  color: var(--st-text-2);\n  cursor: pointer;\n  padding: 2px 6px;\n  font-size: 16px;\n  line-height: 1;\n  border-radius: 4px;\n  \n  &:hover {\n    color: var(--st-text-1);\n    background-color: color-mix(in srgb, var(--st-text-1) 10%, transparent);\n  }\n}\n\n.toggle-icon {\n  font-size: 12px;\n  color: #6c757d;\n  transition: transform 0.3s ease;\n\n  &.collapsed {\n    transform: rotate(-90deg);\n  }\n}\n\n// Update existing styles to work with new structure\n.available-fields, .drop-zone {\n  margin-bottom: 16px;\n  \n  &:last-child {\n    margin-bottom: 0;\n  }\n}\n\n.field-item {\n  margin-bottom: 8px;\n  \n  &:last-child {\n    margin-bottom: 0;\n  }\n}\n\n.filter-controls {\n  display: flex;\n  flex-direction: column;\n  gap: 4px;\n  width: 100%;\n  \n  select, input {\n    width: 100%;\n    padding: 4px;\n    \n    border: 1px solid var(--st-color-neutral);\n    background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n    color: var(--st-text-1);\n\n    border-radius: 3px;\n    font-size: 12px;\n    \n    &:focus {\n      outline: none;\n      border-color: #0066cc;\n    }\n  }\n}\n\n.add-filter-section {\n  padding: 8px;\n  border-top: 1px solid #eee;\n  margin-top: 8px;\n}\n\n.column-select-container {\n  display: flex;\n  gap: 8px;\n  align-items: center;\n  width: 100%;\n}\n\n.column-select {\n  flex: 1;\n  padding: 6px 8px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n  font-size: 13px;\n  background-color: white;\n  \n  &:focus {\n    outline: none;\n    border-color: #0066cc;\n  }\n}\n\n.cancel-btn {\n  background: none;\n  border: none;\n  color: #adb5bd;\n  cursor: pointer;\n  padding: 4px;\n  font-size: 16px;\n  line-height: 1;\n  border-radius: 4px;\n  \n  &:hover {\n    color: #495057;\n    background-color: #f8f9fa;\n  }\n}\n\n.add-filter-btn {\n  width: 100%;\n  padding: 6px 12px;\n  background-color: #f8f9fa;\n  border: 1px dashed #adb5bd;\n  border-radius: 4px;\n  color: #495057;\n  cursor: pointer;\n  font-size: 13px;\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  gap: 4px;\n  \n  &:hover {\n    background-color: #e9ecef;\n    border-color: #495057;\n  }\n  \n  &::before {\n    content: '+';\n    font-size: 16px;\n    line-height: 1;\n    margin-right: 4px;\n  }\n}\n\n.add-field-section {\n  display: none;\n}\n\n.field-select-container {\n  position: absolute;\n  top: 47px;\n  right: 4px;\n  width: calc(100% - 25px);\n  background: var(--st-dashboard-bg-1);\n  border: solid 1px var(--st-color-neutral);\n  border-radius: 4px;\n  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.15);\n  padding: 8px;\n  z-index: 1000;\n  display: flex;\n  flex-direction: column;\n  gap: 8px;\n}\n\n.field-search {\n  width: 100%;\n  padding: 6px 8px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n  font-size: 13px;\n  box-sizing: border-box;\n  \n  &:focus {\n    outline: none;\n    border-color: #0066cc;\n  }\n}\n\n.field-select {\n  width: 100%;\n  border-radius: 4px;\n  font-size: 13px;\n  background-color: var(--st-dashboard-bg-1);\n  \n  &.expanded {\n    max-height: 200px;\n    overflow-y: auto;\n\n    option {\n      padding: 6px 8px;\n      color: var(--st-text-1);\n      \n      &:hover {\n        background-color: color-mix(in srgb, var(--st-text-1) 10%, transparent);\n      }\n      \n      &[value=\"\"] {\n        color: #6c757d;\n        font-style: italic;\n      }\n    }\n  }\n  \n  &:focus {\n    outline: none;\n    border-color: #0066cc;\n  }\n}\n\n.cancel-btn {\n  position: absolute;\n  top: 11px;\n  right: 11px;\n  width: 20px;\n  background: none;\n  border: none;\n  color: #adb5bd;\n  cursor: pointer;\n  padding: 4px;\n  font-size: 16px;\n  line-height: 1;\n  border-radius: 4px;\n  \n  &:hover {\n    color: #495057;\n    background-color: #f8f9fa;\n  }\n}\n\n.field-content {\n  display: flex;\n  flex-direction: column;\n  gap: 4px;\n  width: 100%;\n}\n\n.field-controls {\n  display: flex;\n  gap: 4px;\n  align-items: center;\n  \n\n}\n\nselect {\n    font-size: 12px;\n    padding: 2px 4px;\n    border: 1px solid var(--st-color-neutral);\n    border-radius: 3px;\n    background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n    color: var(--st-text-1);\n    \n    &.sort-select {\n      width: 110px;\n    }\n    \n    &.order-select {\n      width: 90px;\n    }\n  }\n\n.field-header {\n  display: flex;\n  align-items: center;\n  justify-content: space-between;\n  margin-bottom: 12px;\n\n  .field-name {\n    /* font-size: 16px;\n    font-weight: 500; */\n  }\n}\n\n.filter-type-selector {\n  display: flex;\n  padding: 2px;\n  border-radius: 6px;\n  border: 1px solid var(--st-color-neutral);\n  margin-bottom: 12px;\n}\n\n.filter-type-btn {\n  flex: 1;\n  padding: 2px 5px;\n  font-size: 14px;\n  border: none;\n  background: transparent;\n  cursor: pointer;\n  border-radius: 4px;\n  color: var(--st-text-2);\n  transition: all 0.2s ease;\n\n  &.active {\n    background: color-mix(in srgb, var(--st-text-1) 15%, transparent);\n    color: var(--st-text-1);\n    font-weight: 500;\n  }\n\n  &:hover:not(.active) {\n    background: color-mix(in srgb, var(--st-text-1) 8%, transparent);\n  }\n}\n\n.values-filter {\n  /* background: #fff; */\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 6px;\n  overflow: hidden;\n}\n\n.values-list {\n  max-height: 300px;\n  overflow-y: auto;\n}\n\n.values-list-header {\n  position: sticky;\n  top: 0;\n  background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n  border-bottom: 1px solid var(--st-color-neutral);\n  padding: 12px;\n  z-index: 1;\n}\n\n.search-container {\n  margin-bottom: 12px;\n}\n\n.values-search {\n  width: 100%;\n  padding: 8px 12px;\n  border: 1px solid #ddd;\n  border-radius: 6px;\n  font-size: 14px;\n  background: #f8f8f8;\n  \n  &:focus {\n    outline: none;\n    border-color: #0066cc;\n    background: #fff;\n  }\n\n  &::placeholder {\n    color: #999;\n  }\n}\n\n.values-actions {\n  display: flex;\n  gap: 8px;\n}\n\n.action-btn {\n  flex: 1;\n  padding: 2px 6px;\n  font-size: 14px;\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 6px;\n  background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n  cursor: pointer;\n  color: var(--st-text-1);\n  transition: all 0.2s ease;\n\n  &:hover {\n    background: color-mix(in srgb, var(--st-text-1) 15%, transparent);\n  }\n}\n\n.values-options {\n  padding: 8px 12px;\n}\n\n.value-option {\n  display: flex;\n  align-items: center;\n  padding: 2px 2px;\n  cursor: pointer;\n  border-radius: 4px;\n  transition: background-color 0.2s ease;\n\n  &:hover {\n    background: color-mix(in srgb, var(--st-text-1) 15%, transparent);\n  }\n}\n\n.checkbox-wrapper {\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  width: 20px;\n  height: 20px;\n  margin-right: 12px;\n\n  input[type=\"checkbox\"] {\n    width: 16px;\n    height: 16px;\n    margin: 0;\n    cursor: pointer;\n  }\n}\n\n.value-label {\n  flex: 1;\n  font-size: 14px;\n  overflow: hidden;\n  text-overflow: ellipsis;\n  white-space: nowrap;\n}\n\n// Scrollbar styling\n.values-list {\n  &::-webkit-scrollbar {\n    width: 8px;\n  }\n\n  &::-webkit-scrollbar-track {\n    background: #f1f1f1;\n    border-radius: 4px;\n  }\n\n  &::-webkit-scrollbar-thumb {\n    background: #ccc;\n    border-radius: 4px;\n\n    &:hover {\n      background: #bbb;\n    }\n  }\n}\n\n// Condition filter styles\n.condition-filter {\n  select, input {\n    width: 100%;\n    /* padding: 8px 12px; */\n    /* border: 1px solid #ddd; */\n    border-radius: 6px;\n    font-size: 14px;\n    margin-bottom: 8px;\n    /* background: #fff; */\n    box-sizing: border-box;\n\n    &:focus {\n      outline: none;\n      border-color: #0066cc;\n    }\n  }\n\n  input {\n    padding: 6px 10px;\n  }\n}\n\n.remove-btn {\n  position: absolute;\n  top: 2px;\n  right: 2px;\n  background: none;\n  border: none;\n  color: #999;\n  font-size: 18px;\n  cursor: pointer;\n  padding: 4px 8px;\n  border-radius: 4px;\n\n  &:hover {\n    color: #666;\n    background: #f0f0f0;\n  }\n}\n\n.formula-input-container {\n  margin-top: 8px;\n  width: 100%;\n}\n\n.formula-input {\n  width: 100%;\n  padding: 8px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n  font-size: 13px;\n  \n  &:focus {\n    outline: none;\n    border-color: #0066cc;\n  }\n  \n  &.error {\n    border-color: #dc3545;\n    background-color: #fff3f3;\n  }\n}\n\n.formula-error {\n  color: #dc3545;\n  font-size: 12px;\n  margin-top: 4px;\n  padding: 4px;\n  background-color: #fff3f3;\n  border-radius: 4px;\n}\n\n.actions-panel {\n  position: sticky;\n  bottom: 0;\n  left: 0;\n  right: 0;\n  padding: 12px;\n  background: var(--st-dashboard-bg-0);\n  border-top: 1px solid var(--st-color-neutral);\n  display: flex;\n  justify-content: center;\n  margin: 0 -6px -6px -6px;\n  border-radius: 0 0 8px 8px;\n  flex-shrink: 0; // Prevent panel from shrinking\n  z-index: 10; // Ensure panel stays above scrolled content\n\n  .export-btn {\n    width: 100%;\n    padding: 8px;\n    background: var(--st-dashboard-bg-2);\n    border: 1px solid var(--st-color-neutral);\n    border-radius: 4px;\n    color: var(--st-text-1);\n    cursor: pointer;\n    font-size: 13px;\n    font-weight: 500;\n    \n    &:hover {\n      background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n    }\n  }\n}\n</style> ",".fields-container {\n  display: flex;\n  flex-direction: column;\n  gap: 16px;\n  width: 300px;\n  background-color: var(--st-dashboard-bg-0);\n  padding: 6px;\n  border-radius: 8px;\n  border: 1px solid var(--st-color-neutral);\n  height: 100%;\n  box-sizing: border-box;\n  position: relative;\n}\n\n.zones-container {\n  display: flex;\n  flex-direction: column;\n  gap: 2px;\n  flex: 1;\n  overflow-y: auto;\n  padding-bottom: 12px;\n}\n.zones-container::-webkit-scrollbar {\n  width: 8px;\n  height: 8px;\n}\n.zones-container::-webkit-scrollbar-track {\n  background: transparent;\n}\n.zones-container::-webkit-scrollbar-thumb {\n  background: var(--st-color-neutral);\n  border-radius: 4px;\n}\n.zones-container::-webkit-scrollbar-thumb:hover {\n  background: color-mix(in srgb, var(--st-color-neutral) 80%, transparent);\n}\n.zones-container::-webkit-scrollbar-corner {\n  background: transparent;\n}\n\n.field-item {\n  position: relative;\n  z-index: 1;\n  padding: 12px 12px 12px 12px;\n  background: #f8f9fa;\n  border: 1px solid #e9ecef;\n  border-radius: 4px;\n  margin-bottom: 8px;\n  cursor: move;\n  display: flex;\n  justify-content: space-between;\n  align-items: flex-start;\n  font-size: 13px;\n}\n.field-item:last-child {\n  margin-bottom: 0;\n}\n.field-item.dropped {\n  background: color-mix(in srgb, var(--st-text-2) 5%, transparent);\n  border-color: var(--st-color-neutral);\n  color: var(--st-text-1);\n}\n.has-no-results .field-item.dropped {\n  background: color-mix(in srgb, #ff0000 15%, transparent);\n  border-color: color-mix(in srgb, #ff0000 35%, transparent);\n}\n.has-no-results .field-item.dropped .filter-controls select, .has-no-results .field-item.dropped .filter-controls input {\n  border-color: color-mix(in srgb, #ff0000 35%, transparent);\n}\n.has-no-results .field-item.dropped .filter-controls select:focus, .has-no-results .field-item.dropped .filter-controls input:focus {\n  border-color: #ef5350;\n}\n.field-item.drag-over-top {\n  border-top: 2px solid #0066cc;\n}\n.field-item.drag-over-bottom {\n  border-bottom: 2px solid #0066cc;\n}\n\n.remove-btn {\n  position: absolute;\n  top: 2px;\n  right: 2px;\n  background: none;\n  border: none;\n  color: #adb5bd;\n  cursor: pointer;\n  padding: 4px;\n  font-size: 16px;\n  line-height: 1;\n  z-index: 2;\n  border-radius: 4px;\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  width: 20px;\n  height: 20px;\n}\n.remove-btn:hover {\n  color: #495057;\n  background-color: rgba(0, 0, 0, 0.05);\n}\n\n.field-content {\n  display: flex;\n  flex-direction: column;\n  gap: 6px;\n  flex: 1;\n  min-width: 0;\n}\n\n.section {\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 4px;\n  background: var(--st-dashboard-bg-1);\n}\n.section .section-header {\n  padding: 5px 10px;\n  background: var(--st-dashboard-bg-2);\n  border-bottom: 1px solid var(--st-color-neutral);\n  cursor: pointer;\n  display: flex;\n  justify-content: space-between;\n  align-items: center;\n  user-select: none;\n}\n.section .section-header:hover {\n  background: color-mix(in srgb, var(--st-dashboard-bg-2), var(--st-text-1) 10%);\n}\n.section .section-header h4 {\n  margin: 0;\n  color: var(--st-text-1);\n  font-size: 14px;\n  font-weight: 600;\n}\n.section .section-content {\n  padding: 7px 5px;\n  transition: max-height 0.3s ease-in-out;\n  overflow: visible;\n  position: relative;\n}\n.section .section-content.collapsed {\n  max-height: 0;\n  padding: 0;\n  overflow: hidden;\n}\n\n.header-controls {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n}\n\n.add-btn {\n  background: none;\n  border: none;\n  color: var(--st-text-2);\n  cursor: pointer;\n  padding: 2px 6px;\n  font-size: 16px;\n  line-height: 1;\n  border-radius: 4px;\n}\n.add-btn:hover {\n  color: var(--st-text-1);\n  background-color: color-mix(in srgb, var(--st-text-1) 10%, transparent);\n}\n\n.toggle-icon {\n  font-size: 12px;\n  color: #6c757d;\n  transition: transform 0.3s ease;\n}\n.toggle-icon.collapsed {\n  transform: rotate(-90deg);\n}\n\n.available-fields, .drop-zone {\n  margin-bottom: 16px;\n}\n.available-fields:last-child, .drop-zone:last-child {\n  margin-bottom: 0;\n}\n\n.field-item {\n  margin-bottom: 8px;\n}\n.field-item:last-child {\n  margin-bottom: 0;\n}\n\n.filter-controls {\n  display: flex;\n  flex-direction: column;\n  gap: 4px;\n  width: 100%;\n}\n.filter-controls select, .filter-controls input {\n  width: 100%;\n  padding: 4px;\n  border: 1px solid var(--st-color-neutral);\n  background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n  color: var(--st-text-1);\n  border-radius: 3px;\n  font-size: 12px;\n}\n.filter-controls select:focus, .filter-controls input:focus {\n  outline: none;\n  border-color: #0066cc;\n}\n\n.add-filter-section {\n  padding: 8px;\n  border-top: 1px solid #eee;\n  margin-top: 8px;\n}\n\n.column-select-container {\n  display: flex;\n  gap: 8px;\n  align-items: center;\n  width: 100%;\n}\n\n.column-select {\n  flex: 1;\n  padding: 6px 8px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n  font-size: 13px;\n  background-color: white;\n}\n.column-select:focus {\n  outline: none;\n  border-color: #0066cc;\n}\n\n.cancel-btn {\n  background: none;\n  border: none;\n  color: #adb5bd;\n  cursor: pointer;\n  padding: 4px;\n  font-size: 16px;\n  line-height: 1;\n  border-radius: 4px;\n}\n.cancel-btn:hover {\n  color: #495057;\n  background-color: #f8f9fa;\n}\n\n.add-filter-btn {\n  width: 100%;\n  padding: 6px 12px;\n  background-color: #f8f9fa;\n  border: 1px dashed #adb5bd;\n  border-radius: 4px;\n  color: #495057;\n  cursor: pointer;\n  font-size: 13px;\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  gap: 4px;\n}\n.add-filter-btn:hover {\n  background-color: #e9ecef;\n  border-color: #495057;\n}\n.add-filter-btn::before {\n  content: \"+\";\n  font-size: 16px;\n  line-height: 1;\n  margin-right: 4px;\n}\n\n.add-field-section {\n  display: none;\n}\n\n.field-select-container {\n  position: absolute;\n  top: 47px;\n  right: 4px;\n  width: calc(100% - 25px);\n  background: var(--st-dashboard-bg-1);\n  border: solid 1px var(--st-color-neutral);\n  border-radius: 4px;\n  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.15);\n  padding: 8px;\n  z-index: 1000;\n  display: flex;\n  flex-direction: column;\n  gap: 8px;\n}\n\n.field-search {\n  width: 100%;\n  padding: 6px 8px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n  font-size: 13px;\n  box-sizing: border-box;\n}\n.field-search:focus {\n  outline: none;\n  border-color: #0066cc;\n}\n\n.field-select {\n  width: 100%;\n  border-radius: 4px;\n  font-size: 13px;\n  background-color: var(--st-dashboard-bg-1);\n}\n.field-select.expanded {\n  max-height: 200px;\n  overflow-y: auto;\n}\n.field-select.expanded option {\n  padding: 6px 8px;\n  color: var(--st-text-1);\n}\n.field-select.expanded option:hover {\n  background-color: color-mix(in srgb, var(--st-text-1) 10%, transparent);\n}\n.field-select.expanded option[value=\"\"] {\n  color: #6c757d;\n  font-style: italic;\n}\n.field-select:focus {\n  outline: none;\n  border-color: #0066cc;\n}\n\n.cancel-btn {\n  position: absolute;\n  top: 11px;\n  right: 11px;\n  width: 20px;\n  background: none;\n  border: none;\n  color: #adb5bd;\n  cursor: pointer;\n  padding: 4px;\n  font-size: 16px;\n  line-height: 1;\n  border-radius: 4px;\n}\n.cancel-btn:hover {\n  color: #495057;\n  background-color: #f8f9fa;\n}\n\n.field-content {\n  display: flex;\n  flex-direction: column;\n  gap: 4px;\n  width: 100%;\n}\n\n.field-controls {\n  display: flex;\n  gap: 4px;\n  align-items: center;\n}\n\nselect {\n  font-size: 12px;\n  padding: 2px 4px;\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 3px;\n  background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n  color: var(--st-text-1);\n}\nselect.sort-select {\n  width: 110px;\n}\nselect.order-select {\n  width: 90px;\n}\n\n.field-header {\n  display: flex;\n  align-items: center;\n  justify-content: space-between;\n  margin-bottom: 12px;\n}\n.field-header .field-name {\n  /* font-size: 16px;\n  font-weight: 500; */\n}\n\n.filter-type-selector {\n  display: flex;\n  padding: 2px;\n  border-radius: 6px;\n  border: 1px solid var(--st-color-neutral);\n  margin-bottom: 12px;\n}\n\n.filter-type-btn {\n  flex: 1;\n  padding: 2px 5px;\n  font-size: 14px;\n  border: none;\n  background: transparent;\n  cursor: pointer;\n  border-radius: 4px;\n  color: var(--st-text-2);\n  transition: all 0.2s ease;\n}\n.filter-type-btn.active {\n  background: color-mix(in srgb, var(--st-text-1) 15%, transparent);\n  color: var(--st-text-1);\n  font-weight: 500;\n}\n.filter-type-btn:hover:not(.active) {\n  background: color-mix(in srgb, var(--st-text-1) 8%, transparent);\n}\n\n.values-filter {\n  /* background: #fff; */\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 6px;\n  overflow: hidden;\n}\n\n.values-list {\n  max-height: 300px;\n  overflow-y: auto;\n}\n\n.values-list-header {\n  position: sticky;\n  top: 0;\n  background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n  border-bottom: 1px solid var(--st-color-neutral);\n  padding: 12px;\n  z-index: 1;\n}\n\n.search-container {\n  margin-bottom: 12px;\n}\n\n.values-search {\n  width: 100%;\n  padding: 8px 12px;\n  border: 1px solid #ddd;\n  border-radius: 6px;\n  font-size: 14px;\n  background: #f8f8f8;\n}\n.values-search:focus {\n  outline: none;\n  border-color: #0066cc;\n  background: #fff;\n}\n.values-search::placeholder {\n  color: #999;\n}\n\n.values-actions {\n  display: flex;\n  gap: 8px;\n}\n\n.action-btn {\n  flex: 1;\n  padding: 2px 6px;\n  font-size: 14px;\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 6px;\n  background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n  cursor: pointer;\n  color: var(--st-text-1);\n  transition: all 0.2s ease;\n}\n.action-btn:hover {\n  background: color-mix(in srgb, var(--st-text-1) 15%, transparent);\n}\n\n.values-options {\n  padding: 8px 12px;\n}\n\n.value-option {\n  display: flex;\n  align-items: center;\n  padding: 2px 2px;\n  cursor: pointer;\n  border-radius: 4px;\n  transition: background-color 0.2s ease;\n}\n.value-option:hover {\n  background: color-mix(in srgb, var(--st-text-1) 15%, transparent);\n}\n\n.checkbox-wrapper {\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  width: 20px;\n  height: 20px;\n  margin-right: 12px;\n}\n.checkbox-wrapper input[type=checkbox] {\n  width: 16px;\n  height: 16px;\n  margin: 0;\n  cursor: pointer;\n}\n\n.value-label {\n  flex: 1;\n  font-size: 14px;\n  overflow: hidden;\n  text-overflow: ellipsis;\n  white-space: nowrap;\n}\n\n.values-list::-webkit-scrollbar {\n  width: 8px;\n}\n.values-list::-webkit-scrollbar-track {\n  background: #f1f1f1;\n  border-radius: 4px;\n}\n.values-list::-webkit-scrollbar-thumb {\n  background: #ccc;\n  border-radius: 4px;\n}\n.values-list::-webkit-scrollbar-thumb:hover {\n  background: #bbb;\n}\n\n.condition-filter select, .condition-filter input {\n  width: 100%;\n  /* padding: 8px 12px; */\n  /* border: 1px solid #ddd; */\n  border-radius: 6px;\n  font-size: 14px;\n  margin-bottom: 8px;\n  /* background: #fff; */\n  box-sizing: border-box;\n}\n.condition-filter select:focus, .condition-filter input:focus {\n  outline: none;\n  border-color: #0066cc;\n}\n.condition-filter input {\n  padding: 6px 10px;\n}\n\n.remove-btn {\n  position: absolute;\n  top: 2px;\n  right: 2px;\n  background: none;\n  border: none;\n  color: #999;\n  font-size: 18px;\n  cursor: pointer;\n  padding: 4px 8px;\n  border-radius: 4px;\n}\n.remove-btn:hover {\n  color: #666;\n  background: #f0f0f0;\n}\n\n.formula-input-container {\n  margin-top: 8px;\n  width: 100%;\n}\n\n.formula-input {\n  width: 100%;\n  padding: 8px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n  font-size: 13px;\n}\n.formula-input:focus {\n  outline: none;\n  border-color: #0066cc;\n}\n.formula-input.error {\n  border-color: #dc3545;\n  background-color: #fff3f3;\n}\n\n.formula-error {\n  color: #dc3545;\n  font-size: 12px;\n  margin-top: 4px;\n  padding: 4px;\n  background-color: #fff3f3;\n  border-radius: 4px;\n}\n\n.actions-panel {\n  position: sticky;\n  bottom: 0;\n  left: 0;\n  right: 0;\n  padding: 12px;\n  background: var(--st-dashboard-bg-0);\n  border-top: 1px solid var(--st-color-neutral);\n  display: flex;\n  justify-content: center;\n  margin: 0 -6px -6px -6px;\n  border-radius: 0 0 8px 8px;\n  flex-shrink: 0;\n  z-index: 10;\n}\n.actions-panel .export-btn {\n  width: 100%;\n  padding: 8px;\n  background: var(--st-dashboard-bg-2);\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 4px;\n  color: var(--st-text-1);\n  cursor: pointer;\n  font-size: 13px;\n  font-weight: 500;\n}\n.actions-panel .export-btn:hover {\n  background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n}\n\n/*# sourceMappingURL=st-pivottable-controls.vue.map */"]}, media: undefined });

    };
    /* scoped */
    const __vue_scope_id__$2 = "data-v-6f40c4a4";
    /* module identifier */
  ...
  var script = {
      ...
      props: {
          ...
          rows: {
              type: Array,
              default: () => [],
              validator: (value) => value.every(item => typeof item === 'object' && item.field)
          },
          columns: {
              type: Array,
              default: () => [],
          ...
          values: {
              type: Array,
              default: () => [],
              validator: (value) => value.every(item => typeof item === 'object' && item.field && item.aggregation)
          },
          filters: {
              type: Array,
              default: () => [],
              validator: (value) => value.every(item => {
                  ...
                  const typeOfColumn = typeof item.column;    
                  ...
                  if( typeOfColumn !== 'string' ) {
                      isValid = false;
                      console.log( "filter_validation:typeOfColumn: ", typeOfColumn );
                  }
      ...
      methods: {
          ...
          generateEmptyHeaders() {
              ...
              if (this.config.columns.length) {
                  ...
                  headers.push([{
                      ...
                      label: this.config.values[0].field,
                      colspan: 1,
                      depth: 0
                  }]);
              }
          ...
          isExpanded(rowId) {
              return this.expandedRowsArray.includes(rowId);
          },
      ...
      watch: {
          ...
          data: {
              immediate: true,
              handler(newData) {
                  this.filteredData = this.normalizeData(newData);
                  this.updatePivotData();
              }
          },
          filters: {
        ...
        _c(
          ...
          [
            _c("div", { staticClass: "table-container" }, [
              !_vm.hasNoResults && _vm.pivotData.rows.length
                ? _c("table", [
                    _c(
                      "thead",
                      _vm._l(
  ...
  __vue_render__._withStripped = true;
    ...
    const __vue_inject_styles__ = function (inject) {
      if (!inject) return
      inject("data-v-86206f80_0", { source: "@charset \"UTF-8\";\n.pivot-table[data-v-86206f80] {\n  padding: 20px;\n  /* font-family: Inter, Arial, sans-serif; */\n  height: 100%;\n  display: flex;\n  flex-direction: column;\n  box-sizing: border-box;\n}\n.pivot-table-layout[data-v-86206f80] {\n  display: flex;\n  gap: 20px;\n  align-items: stretch;\n  flex: 1;\n  min-height: 0;\n}\n.pivot-table-layout .table-container[data-v-86206f80] {\n  flex: 1;\n  overflow: auto;\n  min-height: 0;\n  border: solid 1px var(--st-color-neutral);\n  border-radius: 8px;\n  display: flex;\n  flex-direction: column;\n}\n.pivot-table-layout .table-container[data-v-86206f80]::-webkit-scrollbar {\n  width: 8px;\n  height: 8px;\n}\n.pivot-table-layout .table-container[data-v-86206f80]::-webkit-scrollbar-track {\n  background: transparent;\n}\n.pivot-table-layout .table-container[data-v-86206f80]::-webkit-scrollbar-thumb {\n  background: var(--st-color-neutral);\n  border-radius: 4px;\n}\n.pivot-table-layout .table-container[data-v-86206f80]::-webkit-scrollbar-thumb:hover {\n  background: color-mix(in srgb, var(--st-color-neutral) 80%, transparent);\n}\n.pivot-table-layout .table-container[data-v-86206f80]::-webkit-scrollbar-corner {\n  background: transparent;\n}\n.pivot-table-layout .table-container .table-actions[data-v-86206f80] {\n  flex: 0 0 auto;\n  display: flex;\n  justify-content: flex-end;\n  padding: 8px;\n  background: var(--st-dashboard-bg-1);\n  border-bottom: 1px solid var(--st-color-neutral);\n}\n.pivot-table-layout .table-container .table-actions .export-btn[data-v-86206f80] {\n  padding: 6px 12px;\n  background: var(--st-dashboard-bg-2);\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 4px;\n  color: var(--st-text-1);\n  cursor: pointer;\n  font-size: 13px;\n}\n.pivot-table-layout .table-container .table-actions .export-btn[data-v-86206f80]:hover {\n  background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n}\n.table-container[data-v-86206f80] {\n  flex: 1;\n  overflow: auto;\n  min-height: 0;\n  position: relative;\n}\n.table-container table[data-v-86206f80] {\n  border-collapse: collapse;\n  width: 100%;\n}\n.table-container table th[data-v-86206f80], .table-container table td[data-v-86206f80] {\n  border: 1px solid var(--st-pivottable-col-headers-border);\n  padding: 4px 8px;\n  text-align: right;\n}\n.table-container table th[data-v-86206f80]:first-child, .table-container table td[data-v-86206f80]:first-child {\n  text-align: left;\n  position: sticky;\n  left: 0;\n}\n.table-container table th[data-v-86206f80] {\n  background: #f5f5f5;\n  position: sticky;\n  border-bottom: 1px solid #ddd;\n  height: 37px;\n  box-sizing: border-box;\n}\n.table-container table thead tr:nth-child(1) th[data-v-86206f80] {\n  top: 0;\n  z-index: 3;\n}\n.table-container table thead tr:nth-child(1) th[data-v-86206f80]:first-child {\n  z-index: 4;\n}\n.table-container table thead tr:nth-child(2) th[data-v-86206f80] {\n  top: 37px;\n  z-index: 2;\n}\n.table-container table thead tr:nth-child(3) th[data-v-86206f80] {\n  top: 74px;\n  z-index: 1;\n}\n.table-container table thead tr:nth-child(4) th[data-v-86206f80] {\n  top: 111px;\n  z-index: 1;\n}\n.table-container table thead tr:nth-child(5) th[data-v-86206f80] {\n  top: 148px;\n  z-index: 1;\n}\n.table-container table thead tr:nth-child(6) th[data-v-86206f80] {\n  top: 185px;\n  z-index: 1;\n}\n.table-container table tbody td[data-v-86206f80]:first-child {\n  z-index: 1;\n}\n.table-container table tr.subtotal-row td[data-v-86206f80] {\n  border-top: 2px solid #e9ecef;\n  border-bottom: 1px solid #dee2e6;\n  background: #f8f9fa;\n}\n.table-container table tr.subtotal-row td[data-v-86206f80]:first-child {\n  background: #f8f9fa;\n}\n.table-container table tr.total-row td[data-v-86206f80] {\n  border-top: 2px solid #dee2e6;\n  padding-top: 12px;\n  padding-bottom: 12px;\n  background: #f1f3f5;\n}\n.table-container table tr.total-row td[data-v-86206f80]:first-child {\n  background: #f1f3f5;\n}\n.table-container table tr.total-row:last-child td[data-v-86206f80] {\n  border-bottom: 2px solid #dee2e6;\n}\n.field-content[data-v-86206f80] {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n  flex: 1;\n}\n.field-content select[data-v-86206f80] {\n  padding: 2px;\n  border: 1px solid #ddd;\n  border-radius: 3px;\n}\n.sortable-header[data-v-86206f80] {\n  cursor: pointer;\n  user-select: none;\n}\n.sortable-header[data-v-86206f80]:hover {\n  background: #e9e9e9;\n}\n.sortable-header.sorted[data-v-86206f80] {\n  background: #f0f0f0;\n}\n.sort-indicator[data-v-86206f80] {\n  margin-left: 4px;\n  font-size: 0.8em;\n}\n.totals-row[data-v-86206f80] {\n  border-top: 2px solid var(--st-pivottable-col-headers-border);\n}\n.row-label[data-v-86206f80] {\n  white-space: nowrap;\n  cursor: pointer;\n}\n.row-label[data-v-86206f80]:hover {\n  background-color: #f5f5f5;\n}\n.row-content[data-v-86206f80] {\n  display: flex;\n  align-items: center;\n  gap: 4px;\n}\n.expand-btn[data-v-86206f80] {\n  background: none;\n  border: none;\n  cursor: pointer;\n  padding: 0;\n  font-size: 12px;\n  color: #666;\n  width: 20px;\n  height: 20px;\n  display: flex;\n  align-items: center;\n  justify-content: center;\n}\n.expand-btn[data-v-86206f80]:hover {\n  background: #eee;\n  border-radius: 3px;\n}\n.column-header[data-v-86206f80] {\n  background: #f5f5f5;\n  text-align: center !important;\n  border-bottom: 1px solid #ddd;\n}\n.column-header.sortable-header[data-v-86206f80] {\n  cursor: pointer;\n  user-select: none;\n}\n.column-header.sortable-header[data-v-86206f80]:hover {\n  background: #e9e9e9;\n}\n.column-header.sortable-header.sorted[data-v-86206f80] {\n  background: #f0f0f0;\n}\n.row-header[data-v-86206f80] {\n  background: #f5f5f5;\n  text-align: left !important;\n  font-weight: bold;\n  border-right: 2px solid #ddd;\n}\ntr.expanded > td[data-v-86206f80] {\n  background-color: #fafafa;\n}\n.header-content[data-v-86206f80] {\n  display: flex;\n  align-items: center;\n  justify-content: space-between;\n  gap: 8px;\n}\n.header-controls[data-v-86206f80] {\n  display: flex;\n  align-items: center;\n  gap: 4px;\n}\n.column-header[data-v-86206f80] {\n  position: relative;\n}\n.pivot-table.is-dragging .drop-zone.drag-over .field-item[data-v-86206f80] {\n  pointer-events: none;\n}\n.value-cell.drillable[data-v-86206f80] {\n  cursor: pointer;\n  position: relative;\n}\n.value-cell.drillable[data-v-86206f80]:hover {\n  background-color: #f0f7ff;\n}\n.value-cell.drillable[data-v-86206f80]:hover::after {\n  content: \"🔍\";\n  position: absolute;\n  top: 2px;\n  right: 2px;\n  font-size: 10px;\n}\n.drill-modal-backdrop[data-v-86206f80] {\n  position: fixed;\n  top: 0;\n  left: 0;\n  right: 0;\n  bottom: 0;\n  background: rgba(0, 0, 0, 0.5);\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  z-index: 1000;\n}\n.drill-modal[data-v-86206f80] {\n  background: var(--st-dashboard-bg-1);\n  color: var(--st-text-1);\n  border-radius: 8px;\n  width: 90%;\n  max-width: 1200px;\n  max-height: 90vh;\n  display: flex;\n  flex-direction: column;\n  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.15);\n}\n.drill-modal .drill-modal-header[data-v-86206f80] {\n  padding: 16px;\n  border-bottom: 1px solid #eee;\n  display: flex;\n  justify-content: space-between;\n  align-items: center;\n}\n.drill-modal .drill-modal-header h3[data-v-86206f80] {\n  margin: 0;\n  font-size: 18px;\n  color: var(--st-text-1);\n}\n.drill-modal .drill-modal-header .close-btn[data-v-86206f80] {\n  background: none;\n  border: none;\n  font-size: 24px;\n  cursor: pointer;\n  color: #666;\n}\n.drill-modal .drill-modal-header .close-btn[data-v-86206f80]:hover {\n  color: var(--st-text-1);\n}\n.drill-modal .drill-modal-body[data-v-86206f80] {\n  padding: 16px;\n  overflow: auto;\n}\n.drill-modal .drill-modal-body table[data-v-86206f80] {\n  width: 100%;\n  border-collapse: collapse;\n}\n.drill-modal .drill-modal-body table th[data-v-86206f80], .drill-modal .drill-modal-body table td[data-v-86206f80] {\n  padding: 8px;\n  border: 1px solid var(--st-pivottable-col-headers-border);\n  text-align: left;\n}\n.drill-modal .drill-modal-body table th[data-v-86206f80] {\n  background: var(--st-dashboard-bg-2);\n  font-weight: 600;\n}\n.drill-modal .drill-modal-body table tr[data-v-86206f80]:nth-child(even) {\n  background: #f9f9f9;\n}\n.drill-modal .drill-modal-body table tr[data-v-86206f80]:hover {\n  background: #f0f7ff;\n}\n.no-results[data-v-86206f80] {\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  height: 200px;\n  font-size: 14px;\n  background: color-mix(in srgb, var(--st-text-1) 15%, transparent);\n  color: var(--st-text-1);\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 4px;\n  margin: 20px 0;\n}\n.filter-type-toggle[data-v-86206f80] {\n  margin: 5px 0;\n}\n.toggle-btn[data-v-86206f80] {\n  padding: 4px 8px;\n  border: 1px solid #ddd;\n  background: #f5f5f5;\n  cursor: pointer;\n  border-radius: 4px;\n}\n.toggle-btn.active[data-v-86206f80] {\n  background: #e0e0e0;\n  border-color: #ccc;\n}\n.values-filter[data-v-86206f80] {\n  margin-top: 8px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n}\n.values-list[data-v-86206f80] {\n  max-height: 200px;\n  overflow-y: auto;\n}\n.values-list-header[data-v-86206f80] {\n  padding: 8px;\n  border-bottom: 1px solid #ddd;\n  background: #f5f5f5;\n}\n.values-search[data-v-86206f80] {\n  width: 100%;\n  padding: 4px;\n  margin-bottom: 8px;\n}\n.values-actions[data-v-86206f80] {\n  display: flex;\n  gap: 8px;\n}\n.values-actions button[data-v-86206f80] {\n  padding: 2px 6px;\n  font-size: 12px;\n}\n.values-options[data-v-86206f80] {\n  padding: 8px;\n}\n.value-option[data-v-86206f80] {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n  padding: 4px 0;\n  cursor: pointer;\n}\n.value-option[data-v-86206f80]:hover {\n  background: #f5f5f5;\n}\n.field-header[data-v-86206f80] {\n  display: flex;\n  align-items: center;\n  justify-content: space-between;\n  margin-bottom: 8px;\n}\n.field-name[data-v-86206f80] {\n  font-weight: 500;\n}\n.value-label[data-v-86206f80] {\n  overflow: hidden;\n  text-overflow: ellipsis;\n  white-space: nowrap;\n}\n.values-filter[data-v-86206f80] {\n  margin-top: 8px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n  background: white;\n}\n.values-list[data-v-86206f80] {\n  max-height: 200px;\n  overflow-y: auto;\n}\n.values-list-header[data-v-86206f80] {\n  position: sticky;\n  top: 0;\n  padding: 8px;\n  border-bottom: 1px solid #ddd;\n  background: #f5f5f5;\n  z-index: 1;\n}\n.values-search[data-v-86206f80] {\n  width: 100%;\n  padding: 6px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n  margin-bottom: 8px;\n}\n.values-actions[data-v-86206f80] {\n  display: flex;\n  gap: 8px;\n}\n.values-actions button[data-v-86206f80] {\n  padding: 4px 8px;\n  font-size: 12px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n  background: white;\n  cursor: pointer;\n}\n.values-actions button[data-v-86206f80]:hover {\n  background: #f0f0f0;\n}\n.values-options[data-v-86206f80] {\n  padding: 8px;\n}\n.value-option[data-v-86206f80] {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n  padding: 6px 8px;\n  cursor: pointer;\n  border-radius: 4px;\n}\n.value-option[data-v-86206f80]:hover {\n  background: #f5f5f5;\n}\n.value-option input[type=checkbox][data-v-86206f80] {\n  margin: 0;\n}\n\n/*# sourceMappingURL=st-pivottable.vue.map */", map: {"version":3,"sources":["st-pivottable.vue","/Users/daniel/Documents/work/stipple_app/pivot-table/lib/src/components/st-pivottable.vue"],"names":[],"mappings":"AAAA,gBAAgB;AC2qBhB;EACA,aAAA;EACA,2CAAA;EACA,YAAA;EACA,aAAA;EACA,sBAAA;EACA,sBAAA;ADzqBA;AC4qBA;EACA,aAAA;EACA,SAAA;EACA,oBAAA;EACA,OAAA;EACA,aAAA;ADzqBA;AC2qBA;EACA,OAAA;EACA,cAAA;EACA,aAAA;EACA,yCAAA;EACA,kBAAA;EACA,aAAA;EACA,sBAAA;ADzqBA;AC4qBA;EACA,UAAA;EACA,WAAA;AD1qBA;AC6qBA;EACA,uBAAA;AD3qBA;AC8qBA;EACA,mCAAA;EACA,kBAAA;AD5qBA;AC8qBA;EACA,wEAAA;AD5qBA;AC+qBA;EACA,uBAAA;AD7qBA;ACgrBA;EACA,cAAA;EACA,aAAA;EACA,yBAAA;EACA,YAAA;EACA,oCAAA;EACA,gDAAA;AD9qBA;ACgrBA;EACA,iBAAA;EACA,oCAAA;EACA,yCAAA;EACA,kBAAA;EACA,uBAAA;EACA,eAAA;EACA,eAAA;AD9qBA;ACgrBA;EACA,gEAAA;AD9qBA;ACqrBA;EACA,OAAA;EACA,cAAA;EACA,aAAA;EACA,kBAAA;ADlrBA;ACorBA;EACA,yBAAA;EACA,WAAA;ADlrBA;ACorBA;EACA,yDAAA;EACA,gBAAA;EACA,iBAAA;ADlrBA;ACorBA;EACA,gBAAA;EACA,gBAAA;EACA,OAAA;ADlrBA;ACurBA;EACA,mBAAA;EACA,gBAAA;EACA,6BAAA;EACA,YAAA;EACA,sBAAA;ADrrBA;AC0rBA;EACA,MAAA;EACA,UAAA;ADxrBA;AC2rBA;EACA,UAAA;ADzrBA;ACgsBA;EACA,SAAA;EACA,UAAA;AD9rBA;ACosBA;EACA,SAAA;EACA,UAAA;ADlsBA;ACusBA;EACA,UAAA;EACA,UAAA;ADrsBA;AC0sBA;EACA,UAAA;EACA,UAAA;ADxsBA;AC6sBA;EACA,UAAA;EACA,UAAA;AD3sBA;ACgtBA;EACA,UAAA;AD9sBA;ACktBA;EACA,6BAAA;EACA,gCAAA;EACA,mBAAA;ADhtBA;ACktBA;EACA,mBAAA;ADhtBA;ACstBA;EACA,6BAAA;EACA,iBAAA;EACA,oBAAA;EACA,mBAAA;ADptBA;ACstBA;EACA,mBAAA;ADptBA;ACwtBA;EACA,gCAAA;ADttBA;AC4tBA;EACA,aAAA;EACA,mBAAA;EACA,QAAA;EACA,OAAA;ADztBA;AC2tBA;EACA,YAAA;EACA,sBAAA;EACA,kBAAA;ADztBA;AC6tBA;EACA,eAAA;EACA,iBAAA;AD1tBA;AC4tBA;EACA,mBAAA;AD1tBA;AC6tBA;EACA,mBAAA;AD3tBA;AC+tBA;EACA,gBAAA;EACA,gBAAA;AD5tBA;AC+tBA;EACA,6DAAA;AD5tBA;AC+tBA;EACA,mBAAA;EACA,eAAA;AD5tBA;AC8tBA;EACA,yBAAA;AD5tBA;ACguBA;EACA,aAAA;EACA,mBAAA;EACA,QAAA;AD7tBA;ACguBA;EACA,gBAAA;EACA,YAAA;EACA,eAAA;EACA,UAAA;EACA,eAAA;EACA,WAAA;EACA,WAAA;EACA,YAAA;EACA,aAAA;EACA,mBAAA;EACA,uBAAA;AD7tBA;AC+tBA;EACA,gBAAA;EACA,kBAAA;AD7tBA;ACiuBA;EACA,mBAAA;EACA,6BAAA;EACA,6BAAA;AD9tBA;ACguBA;EACA,eAAA;EACA,iBAAA;AD9tBA;ACguBA;EACA,mBAAA;AD9tBA;ACiuBA;EACA,mBAAA;AD/tBA;ACouBA;EACA,mBAAA;EACA,2BAAA;EACA,iBAAA;EACA,4BAAA;ADjuBA;ACquBA;EACA,yBAAA;ADluBA;ACsuBA;EACA,aAAA;EACA,mBAAA;EACA,8BAAA;EACA,QAAA;ADnuBA;ACsuBA;EACA,aAAA;EACA,mBAAA;EACA,QAAA;ADnuBA;ACsuBA;EACA,kBAAA;ADnuBA;AC2uBA;EACA,oBAAA;ADxuBA;ACgvBA;EACA,eAAA;EACA,kBAAA;AD7uBA;AC+uBA;EACA,yBAAA;AD7uBA;AC+uBA;EACA,aAAA;EACA,kBAAA;EACA,QAAA;EACA,UAAA;EACA,eAAA;AD7uBA;ACmvBA;EACA,eAAA;EACA,MAAA;EACA,OAAA;EACA,QAAA;EACA,SAAA;EACA,8BAAA;EACA,aAAA;EACA,mBAAA;EACA,uBAAA;EACA,aAAA;ADhvBA;ACmvBA;EACA,oCAAA;EACA,uBAAA;EACA,kBAAA;EACA,UAAA;EACA,iBAAA;EACA,gBAAA;EACA,aAAA;EACA,sBAAA;EACA,yCAAA;ADhvBA;ACkvBA;EACA,aAAA;EACA,6BAAA;EACA,aAAA;EACA,8BAAA;EACA,mBAAA;ADhvBA;ACkvBA;EACA,SAAA;EACA,eAAA;EACA,uBAAA;ADhvBA;ACmvBA;EACA,gBAAA;EACA,YAAA;EACA,eAAA;EACA,eAAA;EACA,WAAA;ADjvBA;ACmvBA;EACA,uBAAA;ADjvBA;ACsvBA;EACA,aAAA;EACA,cAAA;ADpvBA;ACsvBA;EACA,WAAA;EACA,yBAAA;ADpvBA;ACsvBA;EACA,YAAA;EACA,yDAAA;EACA,gBAAA;ADpvBA;ACuvBA;EACA,oCAAA;EACA,gBAAA;ADrvBA;ACwvBA;EACA,mBAAA;ADtvBA;ACyvBA;EACA,mBAAA;ADvvBA;AC6vBA;EACA,aAAA;EACA,mBAAA;EACA,uBAAA;EACA,aAAA;EACA,eAAA;EACA,iEAAA;EACA,uBAAA;EACA,yCAAA;EACA,kBAAA;EACA,cAAA;AD1vBA;AC6vBA;EACA,aAAA;AD1vBA;AC6vBA;EACA,gBAAA;EACA,sBAAA;EACA,mBAAA;EACA,eAAA;EACA,kBAAA;AD1vBA;AC6vBA;EACA,mBAAA;EACA,kBAAA;AD1vBA;AC6vBA;EACA,eAAA;EACA,sBAAA;EACA,kBAAA;AD1vBA;AC6vBA;EACA,iBAAA;EACA,gBAAA;AD1vBA;AC6vBA;EACA,YAAA;EACA,6BAAA;EACA,mBAAA;AD1vBA;AC6vBA;EACA,WAAA;EACA,YAAA;EACA,kBAAA;AD1vBA;AC6vBA;EACA,aAAA;EACA,QAAA;AD1vBA;AC6vBA;EACA,gBAAA;EACA,eAAA;AD1vBA;AC6vBA;EACA,YAAA;AD1vBA;AC6vBA;EACA,aAAA;EACA,mBAAA;EACA,QAAA;EACA,cAAA;EACA,eAAA;AD1vBA;AC6vBA;EACA,mBAAA;AD1vBA;AC6vBA;EACA,aAAA;EACA,mBAAA;EACA,8BAAA;EACA,kBAAA;AD1vBA;AC6vBA;EACA,gBAAA;AD1vBA;AC6vBA;EACA,gBAAA;EACA,uBAAA;EACA,mBAAA;AD1vBA;AC6vBA;EACA,eAAA;EACA,sBAAA;EACA,kBAAA;EACA,iBAAA;AD1vBA;AC6vBA;EACA,iBAAA;EACA,gBAAA;AD1vBA;AC6vBA;EACA,gBAAA;EACA,MAAA;EACA,YAAA;EACA,6BAAA;EACA,mBAAA;EACA,UAAA;AD1vBA;AC6vBA;EACA,WAAA;EACA,YAAA;EACA,sBAAA;EACA,kBAAA;EACA,kBAAA;AD1vBA;AC6vBA;EACA,aAAA;EACA,QAAA;AD1vBA;AC6vBA;EACA,gBAAA;EACA,eAAA;EACA,sBAAA;EACA,kBAAA;EACA,iBAAA;EACA,eAAA;AD1vBA;AC6vBA;EACA,mBAAA;AD1vBA;AC6vBA;EACA,YAAA;AD1vBA;AC6vBA;EACA,aAAA;EACA,mBAAA;EACA,QAAA;EACA,gBAAA;EACA,eAAA;EACA,kBAAA;AD1vBA;AC6vBA;EACA,mBAAA;AD1vBA;AC6vBA;EACA,SAAA;AD1vBA;;AAEA,4CAA4C","file":"st-pivottable.vue","sourcesContent":["@charset \"UTF-8\";\n.pivot-table {\n  padding: 20px;\n  /* font-family: Inter, Arial, sans-serif; */\n  height: 100%;\n  display: flex;\n  flex-direction: column;\n  box-sizing: border-box;\n}\n\n.pivot-table-layout {\n  display: flex;\n  gap: 20px;\n  align-items: stretch;\n  flex: 1;\n  min-height: 0;\n}\n.pivot-table-layout .table-container {\n  flex: 1;\n  overflow: auto;\n  min-height: 0;\n  border: solid 1px var(--st-color-neutral);\n  border-radius: 8px;\n  display: flex;\n  flex-direction: column;\n}\n.pivot-table-layout .table-container::-webkit-scrollbar {\n  width: 8px;\n  height: 8px;\n}\n.pivot-table-layout .table-container::-webkit-scrollbar-track {\n  background: transparent;\n}\n.pivot-table-layout .table-container::-webkit-scrollbar-thumb {\n  background: var(--st-color-neutral);\n  border-radius: 4px;\n}\n.pivot-table-layout .table-container::-webkit-scrollbar-thumb:hover {\n  background: color-mix(in srgb, var(--st-color-neutral) 80%, transparent);\n}\n.pivot-table-layout .table-container::-webkit-scrollbar-corner {\n  background: transparent;\n}\n.pivot-table-layout .table-container .table-actions {\n  flex: 0 0 auto;\n  display: flex;\n  justify-content: flex-end;\n  padding: 8px;\n  background: var(--st-dashboard-bg-1);\n  border-bottom: 1px solid var(--st-color-neutral);\n}\n.pivot-table-layout .table-container .table-actions .export-btn {\n  padding: 6px 12px;\n  background: var(--st-dashboard-bg-2);\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 4px;\n  color: var(--st-text-1);\n  cursor: pointer;\n  font-size: 13px;\n}\n.pivot-table-layout .table-container .table-actions .export-btn:hover {\n  background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n}\n\n.table-container {\n  flex: 1;\n  overflow: auto;\n  min-height: 0;\n  position: relative;\n}\n.table-container table {\n  border-collapse: collapse;\n  width: 100%;\n}\n.table-container table th, .table-container table td {\n  border: 1px solid var(--st-pivottable-col-headers-border);\n  padding: 4px 8px;\n  text-align: right;\n}\n.table-container table th:first-child, .table-container table td:first-child {\n  text-align: left;\n  position: sticky;\n  left: 0;\n}\n.table-container table th {\n  background: #f5f5f5;\n  position: sticky;\n  border-bottom: 1px solid #ddd;\n  height: 37px;\n  box-sizing: border-box;\n}\n.table-container table thead tr:nth-child(1) th {\n  top: 0;\n  z-index: 3;\n}\n.table-container table thead tr:nth-child(1) th:first-child {\n  z-index: 4;\n}\n.table-container table thead tr:nth-child(2) th {\n  top: 37px;\n  z-index: 2;\n}\n.table-container table thead tr:nth-child(3) th {\n  top: 74px;\n  z-index: 1;\n}\n.table-container table thead tr:nth-child(4) th {\n  top: 111px;\n  z-index: 1;\n}\n.table-container table thead tr:nth-child(5) th {\n  top: 148px;\n  z-index: 1;\n}\n.table-container table thead tr:nth-child(6) th {\n  top: 185px;\n  z-index: 1;\n}\n.table-container table tbody td:first-child {\n  z-index: 1;\n}\n.table-container table tr.subtotal-row td {\n  border-top: 2px solid #e9ecef;\n  border-bottom: 1px solid #dee2e6;\n  background: #f8f9fa;\n}\n.table-container table tr.subtotal-row td:first-child {\n  background: #f8f9fa;\n}\n.table-container table tr.total-row td {\n  border-top: 2px solid #dee2e6;\n  padding-top: 12px;\n  padding-bottom: 12px;\n  background: #f1f3f5;\n}\n.table-container table tr.total-row td:first-child {\n  background: #f1f3f5;\n}\n.table-container table tr.total-row:last-child td {\n  border-bottom: 2px solid #dee2e6;\n}\n\n.field-content {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n  flex: 1;\n}\n.field-content select {\n  padding: 2px;\n  border: 1px solid #ddd;\n  border-radius: 3px;\n}\n\n.sortable-header {\n  cursor: pointer;\n  user-select: none;\n}\n.sortable-header:hover {\n  background: #e9e9e9;\n}\n.sortable-header.sorted {\n  background: #f0f0f0;\n}\n\n.sort-indicator {\n  margin-left: 4px;\n  font-size: 0.8em;\n}\n\n.totals-row {\n  border-top: 2px solid var(--st-pivottable-col-headers-border);\n}\n\n.row-label {\n  white-space: nowrap;\n  cursor: pointer;\n}\n.row-label:hover {\n  background-color: #f5f5f5;\n}\n\n.row-content {\n  display: flex;\n  align-items: center;\n  gap: 4px;\n}\n\n.expand-btn {\n  background: none;\n  border: none;\n  cursor: pointer;\n  padding: 0;\n  font-size: 12px;\n  color: #666;\n  width: 20px;\n  height: 20px;\n  display: flex;\n  align-items: center;\n  justify-content: center;\n}\n.expand-btn:hover {\n  background: #eee;\n  border-radius: 3px;\n}\n\n.column-header {\n  background: #f5f5f5;\n  text-align: center !important;\n  border-bottom: 1px solid #ddd;\n}\n.column-header.sortable-header {\n  cursor: pointer;\n  user-select: none;\n}\n.column-header.sortable-header:hover {\n  background: #e9e9e9;\n}\n.column-header.sortable-header.sorted {\n  background: #f0f0f0;\n}\n\n.row-header {\n  background: #f5f5f5;\n  text-align: left !important;\n  font-weight: bold;\n  border-right: 2px solid #ddd;\n}\n\ntr.expanded > td {\n  background-color: #fafafa;\n}\n\n.header-content {\n  display: flex;\n  align-items: center;\n  justify-content: space-between;\n  gap: 8px;\n}\n\n.header-controls {\n  display: flex;\n  align-items: center;\n  gap: 4px;\n}\n\n.column-header {\n  position: relative;\n}\n\n.pivot-table.is-dragging .drop-zone.drag-over .field-item {\n  pointer-events: none;\n}\n\n.value-cell.drillable {\n  cursor: pointer;\n  position: relative;\n}\n.value-cell.drillable:hover {\n  background-color: #f0f7ff;\n}\n.value-cell.drillable:hover::after {\n  content: \"🔍\";\n  position: absolute;\n  top: 2px;\n  right: 2px;\n  font-size: 10px;\n}\n\n.drill-modal-backdrop {\n  position: fixed;\n  top: 0;\n  left: 0;\n  right: 0;\n  bottom: 0;\n  background: rgba(0, 0, 0, 0.5);\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  z-index: 1000;\n}\n\n.drill-modal {\n  background: var(--st-dashboard-bg-1);\n  color: var(--st-text-1);\n  border-radius: 8px;\n  width: 90%;\n  max-width: 1200px;\n  max-height: 90vh;\n  display: flex;\n  flex-direction: column;\n  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.15);\n}\n.drill-modal .drill-modal-header {\n  padding: 16px;\n  border-bottom: 1px solid #eee;\n  display: flex;\n  justify-content: space-between;\n  align-items: center;\n}\n.drill-modal .drill-modal-header h3 {\n  margin: 0;\n  font-size: 18px;\n  color: var(--st-text-1);\n}\n.drill-modal .drill-modal-header .close-btn {\n  background: none;\n  border: none;\n  font-size: 24px;\n  cursor: pointer;\n  color: #666;\n}\n.drill-modal .drill-modal-header .close-btn:hover {\n  color: var(--st-text-1);\n}\n.drill-modal .drill-modal-body {\n  padding: 16px;\n  overflow: auto;\n}\n.drill-modal .drill-modal-body table {\n  width: 100%;\n  border-collapse: collapse;\n}\n.drill-modal .drill-modal-body table th, .drill-modal .drill-modal-body table td {\n  padding: 8px;\n  border: 1px solid var(--st-pivottable-col-headers-border);\n  text-align: left;\n}\n.drill-modal .drill-modal-body table th {\n  background: var(--st-dashboard-bg-2);\n  font-weight: 600;\n}\n.drill-modal .drill-modal-body table tr:nth-child(even) {\n  background: #f9f9f9;\n}\n.drill-modal .drill-modal-body table tr:hover {\n  background: #f0f7ff;\n}\n\n.no-results {\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  height: 200px;\n  font-size: 14px;\n  background: color-mix(in srgb, var(--st-text-1) 15%, transparent);\n  color: var(--st-text-1);\n  border: 1px solid var(--st-color-neutral);\n  border-radius: 4px;\n  margin: 20px 0;\n}\n\n.filter-type-toggle {\n  margin: 5px 0;\n}\n\n.toggle-btn {\n  padding: 4px 8px;\n  border: 1px solid #ddd;\n  background: #f5f5f5;\n  cursor: pointer;\n  border-radius: 4px;\n}\n\n.toggle-btn.active {\n  background: #e0e0e0;\n  border-color: #ccc;\n}\n\n.values-filter {\n  margin-top: 8px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n}\n\n.values-list {\n  max-height: 200px;\n  overflow-y: auto;\n}\n\n.values-list-header {\n  padding: 8px;\n  border-bottom: 1px solid #ddd;\n  background: #f5f5f5;\n}\n\n.values-search {\n  width: 100%;\n  padding: 4px;\n  margin-bottom: 8px;\n}\n\n.values-actions {\n  display: flex;\n  gap: 8px;\n}\n\n.values-actions button {\n  padding: 2px 6px;\n  font-size: 12px;\n}\n\n.values-options {\n  padding: 8px;\n}\n\n.value-option {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n  padding: 4px 0;\n  cursor: pointer;\n}\n\n.value-option:hover {\n  background: #f5f5f5;\n}\n\n.field-header {\n  display: flex;\n  align-items: center;\n  justify-content: space-between;\n  margin-bottom: 8px;\n}\n\n.field-name {\n  font-weight: 500;\n}\n\n.value-label {\n  overflow: hidden;\n  text-overflow: ellipsis;\n  white-space: nowrap;\n}\n\n.values-filter {\n  margin-top: 8px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n  background: white;\n}\n\n.values-list {\n  max-height: 200px;\n  overflow-y: auto;\n}\n\n.values-list-header {\n  position: sticky;\n  top: 0;\n  padding: 8px;\n  border-bottom: 1px solid #ddd;\n  background: #f5f5f5;\n  z-index: 1;\n}\n\n.values-search {\n  width: 100%;\n  padding: 6px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n  margin-bottom: 8px;\n}\n\n.values-actions {\n  display: flex;\n  gap: 8px;\n}\n\n.values-actions button {\n  padding: 4px 8px;\n  font-size: 12px;\n  border: 1px solid #ddd;\n  border-radius: 4px;\n  background: white;\n  cursor: pointer;\n}\n\n.values-actions button:hover {\n  background: #f0f0f0;\n}\n\n.values-options {\n  padding: 8px;\n}\n\n.value-option {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n  padding: 6px 8px;\n  cursor: pointer;\n  border-radius: 4px;\n}\n\n.value-option:hover {\n  background: #f5f5f5;\n}\n\n.value-option input[type=checkbox] {\n  margin: 0;\n}\n\n/*# sourceMappingURL=st-pivottable.vue.map */","<template>\n    <div class=\"pivot-table\" :class=\"{ 'is-dragging': isDragging }\">\n      <div class=\"pivot-table-layout\">\n        <div class=\"table-container\">\n          <table v-if=\"!hasNoResults && pivotData.rows.length\">\n              <thead>\n                  <!-- Row Headers -->\n                  <tr v-for=\"(headerRow, rowIndex) in pivotData.headers\" :key=\"rowIndex\">\n                      <!-- Row header column -->\n                      <th \n                          v-if=\"config.rows.length && rowIndex === 0\" \n                          :rowspan=\"pivotData.headers.length\"\n                          class=\"row-header\"\n                      >\n                          {{ config.rows.map(r => r.field).join(' - ') }}\n                      </th>\n                      <!-- Column Headers -->\n                          <th  v-for=\"header in headerRow\" :key=\"header.id\"\n                              :colspan=\"header.colspan\"\n                              class=\"column-header\"\n                              :class=\"{\n                                  'sortable-header': rowIndex === pivotData.headers.length - 1,\n                                  'sorted': sortConfig.column === header.id,\n                                  'sorted-asc': sortConfig.column === header.id && sortConfig.direction === 'asc',\n                                  'sorted-desc': sortConfig.column === header.id && sortConfig.direction === 'desc'\n                              }\"\n                              @click=\"rowIndex === pivotData.headers.length - 1 && toggleSort(header)\"\n                          >\n                              <div class=\"header-content\">\n                                  <span>{{ header.label }}</span>\n                                  <div class=\"header-controls\" v-if=\"rowIndex === pivotData.headers.length - 1\">\n                                  </div>\n                              </div>\n                          </th>\n                  </tr>\n              </thead>\n              <tbody>\n                  <recursive-pivot-rows\n                      v-for=\"row in flattenedRows\"\n                      :key=\"row.id\"\n                      :row=\"row\"\n                      :config=\"config\"\n                      :expanded-rows=\"expandedRowsArray\"\n                      @toggle-expand=\"toggleExpand\"\n                      @cell-click=\"onCellClick\"\n                  />\n                  \n                  <!-- Totals row -->\n                  <tr v-if=\"pivotData.totals\" class=\"totals-row\">\n                      <td v-if=\"config.rows.length\"><strong>Total</strong></td>\n                      <td v-for=\"cell in pivotData.totals.cells\" :key=\"cell.id\">\n                          <strong>{{ formatValue(cell.value) }}</strong>\n                      </td>\n                  </tr>\n              </tbody>\n          </table>\n          <div v-else-if=\"hasNoResults\" class=\"no-results\">\n            <p>No data matches the current filters</p>\n          </div>\n        </div>\n\n        <st-pivottable-controls\n          :data=\"filteredData\"\n          :rows=\"config.rows\"\n          :columns=\"config.columns\"\n          :values=\"config.values\"\n          :filters=\"filters\"\n          :has-no-results=\"hasNoResults\"\n          @configChanged=\"onConfigChanged\"\n          @filtersChanged=\"onFiltersChanged\"\n          @export=\"exportToCsv\"\n        />\n      </div>\n\n      <DrillThroughModal\n        v-if=\"drillThroughData\"\n        :data=\"drillThroughData\"\n        :columns=\"drillThroughColumns\"\n        :title=\"drillThroughTitle\"\n        @close=\"closeDrillThrough\"\n      />\n    </div>\n</template>\n\n<script>\nimport { PivotProcessor } from '../utils/pivotUtils'\nimport DrillThroughModal from './DrillThroughModal.vue'\nimport StPivottableControls from './st-pivottable-controls.vue'\nimport RecursivePivotRows from './recursive-pivot-rows.vue'\n\nexport default {\n    name: 'st-pivottable',\n    components: {\n        DrillThroughModal,\n        StPivottableControls,\n        RecursivePivotRows\n    },\n    \n    props: {\n        data: {\n            type: [Array, Object],\n            required: true,\n            validator: function(value) {\n                if (Array.isArray(value)) {\n                    // For array format, each item must be an object\n                    return value.every(item => typeof item === 'object' && item !== null);\n                } else if (typeof value === 'object' && value !== null) {\n                    // For column-based format, each property must be an array of the same length\n                    const lengths = Object.values(value).map(arr => Array.isArray(arr) ? arr.length : -1);\n                    return lengths.length > 0 && lengths.every(len => len === lengths[0] && len >= 0);\n                }\n                return false;\n            }\n        },\n        rows: {\n            type: Array,\n            default: () => [],\n            validator: (value) => value.every(item => typeof item === 'object' && item.field)\n        },\n        columns: {\n            type: Array,\n            default: () => [],\n            validator: (value) => value.every(item => typeof item === 'object' && item.field)\n        },\n        values: {\n            type: Array,\n            default: () => [],\n            validator: (value) => value.every(item => typeof item === 'object' && item.field && item.aggregation)\n        },\n        filters: {\n            type: Array,\n            default: () => [],\n            validator: (value) => value.every(item => {\n                const typeOfItem = typeof item;\n                const typeOfColumn = typeof item.column;    \n                const typeOfFilterType = typeof item.filterType;\n\n                let isValid = true;\n\n                if( typeOfItem !== 'object' ){\n                    isValid = false;\n                    console.log( \"filter_validation:typeOfItem: \", typeOfItem );\n                }\n                if( typeOfColumn !== 'string' ) {\n                    isValid = false;\n                    console.log( \"filter_validation:typeOfColumn: \", typeOfColumn );\n                }\n                if( typeOfFilterType !== 'string' ) {\n                    isValid = false;\n                    console.log( \"filter_validation:typeOfFilterType: \", typeOfFilterType );\n                }\n\n                if( isValid ) {\n                    if( item.filterType === 'condition' ) {\n                        if( typeof item.condition !== 'string' ) {\n                            isValid = false;\n                            console.log( \"filter_validation:typeOfCondition: \", typeof item.condition, item.condition );\n                        }else if( item.value === undefined ) {\n                            isValid = false;\n                            console.log( \"filter_validation:value: \", item.value );\n                        }\n                    }else if( item.filterType === 'values' ) {\n                        if( !Array.isArray(item.selectedValues) ) {\n                            isValid = false;\n                            console.log( \"filter_validation:selectedValues: \", Array.isArray(item.selectedValues), item.selectedValues );\n                        }\n                    }\n                }\n\n                return isValid;\n            })\n        }\n    },\n\n    data() {\n        return {\n            config: {\n                rows: (this.rows || []).map(row => ({\n                    ...row,\n                    sortBy: row.sortBy || 'label',\n                    sortOrder: row.sortOrder || 'asc'\n                })),\n                columns: (this.columns || []).map(col => ({\n                    ...col,\n                    sortBy: col.sortBy || 'label',\n                    sortOrder: col.sortOrder || 'asc'\n                })),\n                values: this.values || []\n            },\n            isDragging: false,\n            expandedRowsArray: [],\n            sortConfig: {\n                column: null,\n                direction: 'asc'\n            },\n            pivotData: {\n                headers: [],\n                rows: [],\n                totals: null\n            },\n            drillThroughData: null,\n            drillThroughColumns: [],\n            drillThroughTitle: '',\n            filteredData: this.data || [],\n            filterTypes: {\n                condition: 'Filter by Condition',\n                values: 'Filter by Values'\n            },\n            uniqueValuesCache: {},\n        }\n    },\n\n    computed: {\n        flattenedRows() {\n            const flattened = [];\n            \n            const addRow = (row, isVisible) => {\n                if (isVisible) {\n                    flattened.push(row);\n                }\n                \n                if (row.hasChildren && this.isExpanded(row.id)) {\n                    row.children.forEach(child => {\n                        addRow(child, isVisible);\n                    });\n                }\n            };\n            \n            this.pivotData.rows.forEach(row => {\n                addRow(row, true);\n            });\n            \n            return flattened;\n        },\n\n        hasNoResults() {\n            return this.filteredData && this.filteredData.length === 0;\n        },\n\n        getUniqueValues() {\n            return (column) => {\n                if (!this.uniqueValuesCache[column]) {\n                    const values = new Set();\n                    this.filteredData.forEach(row => {\n                        if (row[column] !== null && row[column] !== undefined) {\n                            values.add(row[column]);\n                        }\n                    });\n                    this.uniqueValuesCache[column] = Array.from(values).sort((a, b) => {\n                        return String(a).localeCompare(String(b));\n                    });\n                }\n                return this.uniqueValuesCache[column];\n            };\n        }\n    },\n\n    methods: {\n        normalizeData(data) {\n            if (Array.isArray(data)) {\n                return data;\n            }\n\n            // Convert column-based format to row-based format\n            if (typeof data === 'object' && data !== null) {\n                const columns = Object.keys(data);\n                const rowCount = data[columns[0]].length;\n                const rows = [];\n\n                for (let i = 0; i < rowCount; i++) {\n                    const row = {};\n                    columns.forEach(col => {\n                        row[col] = data[col][i];\n                    });\n                    rows.push(row);\n                }\n\n                return rows;\n            }\n\n            return [];\n        },\n\n        applyFilters() {\n            if (!this.data || !this.data.length) return;\n\n            this.filteredData = this.data.filter(row => {\n                return this.filters.every(filter => {\n                    const value = row[filter.column];\n\n                    if (filter.filterType === 'values') {\n                        return filter.selectedValues.length === 0 || filter.selectedValues.includes(value);\n                    }\n\n                    const filterValue = filter.value;\n\n                    switch (filter.condition) {\n                        case 'equals':\n                            return value == filterValue;\n                        case 'notEquals':\n                            return value != filterValue;\n                        case 'greaterThan':\n                            return value > filterValue;\n                        case 'greaterThanOrEqual':\n                            return value >= filterValue;\n                        case 'lessThan':\n                            return value < filterValue;\n                        case 'lessThanOrEqual':\n                            return value <= filterValue;\n                        case 'contains':\n                            return String(value).toLowerCase().includes(String(filterValue).toLowerCase());\n                        case 'notContains':\n                            return !String(value).toLowerCase().includes(String(filterValue).toLowerCase());\n                        case 'startsWith':\n                            return String(value).toLowerCase().startsWith(String(filterValue).toLowerCase());\n                        case 'endsWith':\n                            return String(value).toLowerCase().endsWith(String(filterValue).toLowerCase());\n                        case 'isEmpty':\n                            return value === null || value === undefined || value === '';\n                        case 'isNotEmpty':\n                            return value !== null && value !== undefined && value !== '';\n                        default:\n                            return true;\n                    }\n                });\n            });\n\n            if (this.filteredData && this.filteredData.length) {\n                const processor = new PivotProcessor(this.filteredData, this.config);\n                this.pivotData = processor.process(this.sortConfig);\n            }\n        },\n\n        updatePivotData() {\n            \n            if (this.filteredData && this.filteredData.length) {\n                const processor = new PivotProcessor(this.filteredData, this.config);\n                this.pivotData = processor.process(this.sortConfig);\n            } else {\n                // Create an empty pivot data structure that maintains headers\n                this.pivotData = {\n                    headers: this.generateEmptyHeaders(),\n                    rows: [],\n                    totals: {\n                        cells: this.config.values.map(value => ({\n                            id: `total_${value.field}_${value.aggregation}`,\n                            value: 0\n                        }))\n                    }\n                };\n            }\n        },\n\n        generateEmptyHeaders() {\n            const headers = [];\n            \n            // If we have column fields, create header structure\n            if (this.config.columns.length) {\n                // Add empty header rows for each column field\n                this.config.columns.forEach((_, index) => {\n                    headers.push([{\n                        id: `empty_header_${index}`,\n                        label: '',\n                        colspan: 1,\n                        depth: index\n                    }]);\n                });\n            } else if (this.config.values.length) {\n                // If no column fields but we have values, add single header row\n                headers.push([{\n                    id: 'empty_value_header',\n                    label: this.config.values[0].field,\n                    colspan: 1,\n                    depth: 0\n                }]);\n            }\n            \n            return headers;\n        },\n\n        onConfigChanged(newConfig) {\n            this.config = newConfig;\n            this.updatePivotData();\n        },\n\n        onFiltersChanged(newFilters) {\n            this.$emit('update:filters', newFilters);\n            this.applyFilters();\n        },\n\n        toggleSort(header) {\n            if (this.sortConfig.column === header.id) {\n                this.sortConfig.direction = this.sortConfig.direction === 'asc' ? 'desc' : 'asc';\n            } else {\n                this.sortConfig.column = header.id;\n                this.sortConfig.direction = 'asc';\n            }\n            this.updatePivotData();\n        },\n\n        formatValue(value) {\n            return typeof value === 'number' ? value.toLocaleString() : value;\n        },\n\n        isExpanded(rowId) {\n            return this.expandedRowsArray.includes(rowId);\n        },\n\n        toggleExpand(rowId) {\n            const index = this.expandedRowsArray.indexOf(rowId);\n            if (index > -1) {\n                this.expandedRowsArray.splice(index, 1);\n            } else {\n                this.expandedRowsArray.push(rowId);\n            }\n        },\n\n        isDrillableCell(cell) {\n            // For now, consider all value cells as drillable\n            // Later we can add more specific conditions\n            return cell && typeof cell.value === 'number';\n        },\n\n        onCellClick(cell, row) {\n            if (!this.isDrillableCell(cell)) return;\n            \n            // Get the column configuration from the cell's position\n            const columnIndex = row.cells.indexOf(cell);\n            console.log('Column index:', columnIndex);\n            console.log('Row:', row);\n            \n            // Get all header levels for this column\n            const headerLevels = [];\n            for (let level = 0; level < this.pivotData.headers.length; level++) {\n                const headerRow = this.pivotData.headers[level];\n                let currentIndex = 0;\n                for (const header of headerRow) {\n                    if (currentIndex <= columnIndex && currentIndex + header.colspan > columnIndex) {\n                        headerLevels.push(header);\n                        break;\n                    }\n                    currentIndex += header.colspan;\n                }\n            }\n            console.log('Header levels:', headerLevels);\n            \n            // Build filters based on row path and column path\n            const filters = {};\n            \n            // Collect all row values up the hierarchy\n            const rowValues = [];\n            let currentRow = row;\n            while (currentRow && currentRow.label) {\n                rowValues.unshift({\n                    label: currentRow.label,\n                    depth: currentRow.depth\n                });\n                currentRow = currentRow.parent;\n            }\n            console.log('Row hierarchy:', rowValues);\n            \n            // Map row values to fields based on depth\n            this.config.rows.forEach((rowField, index) => {\n                const rowValue = rowValues.find(rv => rv.depth === index);\n                if (rowValue) {\n                    filters[rowField.field] = rowValue.label;\n                    console.log(`Adding row filter: ${rowField.field} = ${rowValue.label}`);\n                }\n            });\n            \n            // Add column dimension filters by mapping header levels to column fields\n            this.config.columns.forEach((colField, index) => {\n                const headerLevel = headerLevels[index];\n                if (headerLevel && headerLevel.label) {\n                    filters[colField.field] = headerLevel.label;\n                    console.log(`Adding column filter: ${colField.field} = ${headerLevel.label}`);\n                }\n            });\n            \n            console.log('Applied filters:', filters);\n            \n            // Filter the source data\n            const filteredData = this.filteredData.filter(record => {\n                const matches = Object.entries(filters).every(([field, value]) => {\n                    const match = record[field] === value;\n                    return match;\n                });\n                return matches;\n            });\n            \n            console.log(`Found ${filteredData.length} matching records out of ${this.filteredData.length} total`);\n            \n            // Update modal data\n            const rowPath = rowValues.map(r => r.label).join(' / ');\n            const columnPath = headerLevels.map(h => h.label).join(' / ');\n            this.drillThroughTitle = `Drill Through - ${rowPath} (${columnPath})`;\n            this.drillThroughColumns = Object.keys(this.filteredData[0]);\n            this.drillThroughData = filteredData;\n        },\n\n        closeDrillThrough() {\n            this.drillThroughData = null;\n            this.drillThroughColumns = [];\n            this.drillThroughTitle = '';\n        },\n\n        addNewFilter() {\n            if (this.selectedColumn) {\n                const newFilter = {\n                    column: this.selectedColumn,\n                    filterType: 'condition', // Default to condition for backward compatibility\n                    condition: this.getDefaultCondition(this.selectedColumn),\n                    value: '',\n                    selectedValues: [] // For values filter type\n                };\n                this.filters.push(newFilter);\n                this.selectedColumn = null;\n                this.showColumnSelect = false;\n                this.$emit('filtersChanged', this.filters);\n            }\n        },\n\n        toggleFilterType(filter) {\n            filter.filterType = filter.filterType === 'condition' ? 'values' : 'condition';\n            if (filter.filterType === 'values') {\n                filter.selectedValues = [];\n            } else {\n                filter.condition = this.getDefaultCondition(filter.column);\n                filter.value = '';\n            }\n            this.filterChanged();\n        },\n\n        selectAllValues(filter) {\n            filter.selectedValues = [...this.getUniqueValues(filter.column)];\n            this.filterChanged();\n        },\n\n        clearValues(filter) {\n            filter.selectedValues = [];\n            this.filterChanged();\n        },\n\n        filteredValues(filter) {\n            const values = this.getUniqueValues(filter.column);\n            if (!filter.searchQuery) return values;\n            \n            const searchTerm = filter.searchQuery.toLowerCase();\n            return values.filter(value => \n                String(value).toLowerCase().includes(searchTerm)\n            );\n        },\n\n        exportToCsv() {\n            // Get all header rows\n            const headerRows = this.pivotData.headers.map((headerRow, rowIndex) => {\n                // For each header row, create an array with row field names or empty cells\n                const rowFieldCells = rowIndex === 0 \n                    ? this.config.rows.map(r => r.field)  // First row gets the field names\n                    : Array(this.config.rows.length).fill(''); // Other rows get empty cells\n                \n                // For each header in this row, repeat the label based on its colspan\n                const headerCells = [];\n                headerRow.forEach(header => {\n                    // Repeat the header label for its colspan\n                    for (let i = 0; i < header.colspan; i++) {\n                        headerCells.push(header.label);\n                    }\n                });\n                \n                return [...rowFieldCells, ...headerCells];\n            });\n            \n            // Process rows recursively, including row headers and values\n            const processRow = (row, parentIndent = '') => {\n                const rowData = [];\n                \n                // Add row headers with proper indentation\n                const indent = parentIndent + (row.depth > 0 ? '  ' : '');\n                const rowHeaders = Array(this.config.rows.length).fill('');\n                rowHeaders[row.depth] = indent + row.label;\n                \n                // Add cell values\n                const values = row.cells.map(cell => this.formatValue(cell.value));\n                \n                rowData.push([...rowHeaders, ...values]);\n                \n                // Process children if any\n                if (row.children) {\n                    row.children.forEach(child => {\n                        rowData.push(...processRow(child, indent));\n                    });\n                }\n                \n                return rowData;\n            };\n            \n            // Get all row data\n            const rows = this.pivotData.rows.flatMap(row => processRow(row));\n            \n            // Add totals row if exists\n            if (this.pivotData.totals) {\n                const totalRow = [\n                    'Total',\n                    ...Array(this.config.rows.length - 1).fill(''),\n                    ...this.pivotData.totals.cells.map(cell => this.formatValue(cell.value))\n                ];\n                rows.push(totalRow);\n            }\n            \n            // Combine headers and rows\n            const csvContent = [\n                ...headerRows,  // Include all header rows\n                ...rows\n            ]\n                .map(row => row.map(cell => {\n                    // Escape cells containing commas or quotes\n                    const cellStr = String(cell);\n                    return cellStr.includes(',') || cellStr.includes('\"') \n                        ? `\"${cellStr.replace(/\"/g, '\"\"')}\"` \n                        : cellStr;\n                }).join(','))\n                .join('\\n');\n            \n            // Create and trigger download\n            const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });\n            const link = document.createElement('a');\n            const timestamp = new Date().toISOString().replace(/[:.]/g, '_');\n            link.href = URL.createObjectURL(blob);\n            link.download = `pivot_table_export_${timestamp}.csv`;\n            link.click();\n            URL.revokeObjectURL(link.href);\n        },\n    },\n\n    watch: {\n        rows: {\n            handler(newRows) {\n                this.config.rows = (newRows || []).map(row => ({\n                    ...row,\n                    sortBy: row.sortBy || 'label',\n                    sortOrder: row.sortOrder || 'asc'\n                }));\n                this.updatePivotData();\n            },\n            deep: true\n        },\n        columns: {\n            handler(newColumns) {\n                this.config.columns = (newColumns || []).map(col => ({\n                    ...col,\n                    sortBy: col.sortBy || 'label',\n                    sortOrder: col.sortOrder || 'asc'\n                }));\n                this.updatePivotData();\n            },\n            deep: true\n        },\n        values: {\n            handler(newValues) {\n                this.config.values = newValues;\n                this.updatePivotData();\n            },\n            deep: true\n        },\n        data: {\n            immediate: true,\n            handler(newData) {\n                this.filteredData = this.normalizeData(newData);\n                this.updatePivotData();\n            }\n        },\n        filters: {\n            handler() {\n                this.applyFilters();\n            },\n            deep: true\n        }\n    }\n}\n</script>\n\n<style lang=\"scss\" scoped>\n.pivot-table {\n    padding: 20px;\n    /* font-family: Inter, Arial, sans-serif; */\n    height: 100%;\n    display: flex;\n    flex-direction: column;\n    box-sizing: border-box;\n}\n\n.pivot-table-layout {\n    display: flex;\n    gap: 20px;\n    align-items: stretch;\n    flex: 1;\n    min-height: 0; // Important for flex child scrolling\n    \n    .table-container {\n        flex: 1;\n        overflow: auto;\n        min-height: 0; // Important for flex child scrolling\n        border: solid 1px var(--st-color-neutral);\n        border-radius: 8px;\n        display: flex;\n        flex-direction: column;\n\n        // Improve scrollbar styling\n        &::-webkit-scrollbar {\n            width: 8px;\n            height: 8px;\n        }\n\n        &::-webkit-scrollbar-track {\n            background: transparent;\n        }\n\n        &::-webkit-scrollbar-thumb {\n            background: var(--st-color-neutral);\n            border-radius: 4px;\n\n            &:hover {\n            background: color-mix(in srgb, var(--st-color-neutral) 80%, transparent);\n            }\n        }\n        &::-webkit-scrollbar-corner {\n           background: transparent;\n        }\n        \n        .table-actions {\n            flex: 0 0 auto;\n            display: flex;\n            justify-content: flex-end;\n            padding: 8px;\n            background: var(--st-dashboard-bg-1);\n            border-bottom: 1px solid var(--st-color-neutral);\n            \n            .export-btn {\n                padding: 6px 12px;\n                background: var(--st-dashboard-bg-2);\n                border: 1px solid var(--st-color-neutral);\n                border-radius: 4px;\n                color: var(--st-text-1);\n                cursor: pointer;\n                font-size: 13px;\n                \n                &:hover {\n                    background: color-mix(in srgb, var(--st-text-1) 5%, transparent);\n                }\n            }\n        }\n    }\n}\n\n.table-container {\n    flex: 1;\n    overflow: auto;\n    min-height: 0;\n    position: relative;\n    \n    table {\n        border-collapse: collapse;\n        width: 100%;\n        \n        th, td {\n            border: 1px solid var(--st-pivottable-col-headers-border);\n            padding: 4px 8px;\n            text-align: right;\n            \n            &:first-child {\n                text-align: left;\n                position: sticky;\n                left: 0;\n            }\n        }\n        \n        // All header cells\n        th {\n            background: #f5f5f5;\n            position: sticky;\n            border-bottom: 1px solid #ddd;\n            height: 37px; // Fixed height for header cells\n            box-sizing: border-box;\n        }\n\n        // First row (top-most header)\n        thead tr:nth-child(1) {\n            th {\n                top: 0;\n                z-index: 3;\n\n                // Business Unit - Department cell\n                &:first-child {\n                    z-index: 4;\n                }\n            }\n        }\n\n        // Second row (middle header)\n        thead tr:nth-child(2) {\n            th {\n                top: 37px; // Exactly one header height\n                z-index: 2;\n            }\n        }\n\n        // Third row (bottom header)\n        thead tr:nth-child(3) {\n            th {\n                top: 74px; // Exactly two header heights\n                z-index: 1;\n            }\n        }\n        // Fourth row (bottom header)\n        thead tr:nth-child(4) {\n            th {\n                top: 111px; // Exactly three header heights\n                z-index: 1;\n            }\n        }\n        // Fifth row (bottom header)\n        thead tr:nth-child(5) {\n            th {\n                top: 148px; // Exactly four header heights\n                z-index: 1;\n            }\n        }\n        // Sixth row (bottom header)\n        thead tr:nth-child(6) {\n            th {\n                top: 185px; // Exactly five header heights\n                z-index: 1;\n            }\n        }\n\n        // Body first column\n        tbody td:first-child {\n            z-index: 1;\n        }\n\n        tr.subtotal-row {\n            td {\n                border-top: 2px solid #e9ecef;\n                border-bottom: 1px solid #dee2e6;\n                background: #f8f9fa;\n\n                &:first-child {\n                    background: #f8f9fa;\n                }\n            }\n        }\n\n        tr.total-row {\n            td {\n                border-top: 2px solid #dee2e6;\n                padding-top: 12px;\n                padding-bottom: 12px;\n                background: #f1f3f5;\n\n                &:first-child {\n                    background: #f1f3f5;\n                }\n            }\n\n            &:last-child td {\n                border-bottom: 2px solid #dee2e6;\n            }\n        }\n    }\n}\n\n.field-content {\n    display: flex;\n    align-items: center;\n    gap: 8px;\n    flex: 1;\n    \n    select {\n        padding: 2px;\n        border: 1px solid #ddd;\n        border-radius: 3px;\n    }\n}\n\n.sortable-header {\n    cursor: pointer;\n    user-select: none;\n    \n    &:hover {\n        background: #e9e9e9;\n    }\n    \n    &.sorted {\n        background: #f0f0f0;\n    }\n}\n\n.sort-indicator {\n    margin-left: 4px;\n    font-size: 0.8em;\n}\n\n.totals-row {\n    border-top: 2px solid var(--st-pivottable-col-headers-border);\n}\n\n.row-label {\n    white-space: nowrap;\n    cursor: pointer;\n    \n    &:hover {\n        background-color: #f5f5f5;\n    }\n}\n\n.row-content {\n    display: flex;\n    align-items: center;\n    gap: 4px;\n}\n\n.expand-btn {\n    background: none;\n    border: none;\n    cursor: pointer;\n    padding: 0;\n    font-size: 12px;\n    color: #666;\n    width: 20px;\n    height: 20px;\n    display: flex;\n    align-items: center;\n    justify-content: center;\n    \n    &:hover {\n        background: #eee;\n        border-radius: 3px;\n    }\n}\n\n.column-header {\n    background: #f5f5f5;\n    text-align: center !important;\n    border-bottom: 1px solid #ddd;\n    \n    &.sortable-header {\n        cursor: pointer;\n        user-select: none;\n        \n        &:hover {\n            background: #e9e9e9;\n        }\n        \n        &.sorted {\n            background: #f0f0f0;\n        }\n    }\n}\n\n.row-header {\n    background: #f5f5f5;\n    text-align: left !important;\n    font-weight: bold;\n    border-right: 2px solid #ddd;\n}\n\ntr.expanded {\n    & > td {\n        background-color: #fafafa;\n    }\n}\n\n.header-content {\n    display: flex;\n    align-items: center;\n    justify-content: space-between;\n    gap: 8px;\n}\n\n.header-controls {\n    display: flex;\n    align-items: center;\n    gap: 4px;\n}\n\n.column-header {\n    position: relative;\n    \n}\n\n.pivot-table {\n    &.is-dragging {\n        .drop-zone {\n            &.drag-over {\n                .field-item {\n                    pointer-events: none;\n                }\n            }\n        }\n    }\n}\n\n.value-cell {\n    &.drillable {\n        cursor: pointer;\n        position: relative;\n        \n        &:hover {\n            background-color: #f0f7ff;\n            \n            &::after {\n                content: '🔍';\n                position: absolute;\n                top: 2px;\n                right: 2px;\n                font-size: 10px;\n            }\n        }\n    }\n}\n\n.drill-modal-backdrop {\n    position: fixed;\n    top: 0;\n    left: 0;\n    right: 0;\n    bottom: 0;\n    background: rgba(0, 0, 0, 0.5);\n    display: flex;\n    align-items: center;\n    justify-content: center;\n    z-index: 1000;\n}\n\n.drill-modal {\n    background: var(--st-dashboard-bg-1);\n    color: var(--st-text-1);\n    border-radius: 8px;\n    width: 90%;\n    max-width: 1200px;\n    max-height: 90vh;\n    display: flex;\n    flex-direction: column;\n    box-shadow: 0 2px 8px rgba(0, 0, 0, 0.15);\n    \n    .drill-modal-header {\n        padding: 16px;\n        border-bottom: 1px solid #eee;\n        display: flex;\n        justify-content: space-between;\n        align-items: center;\n        \n        h3 {\n            margin: 0;\n            font-size: 18px;\n            color: var(--st-text-1);\n        }\n        \n        .close-btn {\n            background: none;\n            border: none;\n            font-size: 24px;\n            cursor: pointer;\n            color: #666;\n            \n            &:hover {\n                color: var(--st-text-1);\n            }\n        }\n    }\n    \n    .drill-modal-body {\n        padding: 16px;\n        overflow: auto;\n        \n        table {\n            width: 100%;\n            border-collapse: collapse;\n            \n            th, td {\n                padding: 8px;\n                border: 1px solid var(--st-pivottable-col-headers-border);\n                text-align: left;\n            }\n            \n            th {\n                background: var(--st-dashboard-bg-2);\n                font-weight: 600;\n            }\n            \n            tr:nth-child(even) {\n                background: #f9f9f9;\n            }\n            \n            tr:hover {\n                background: #f0f7ff;\n            }\n        }\n    }\n}\n\n.no-results {\n    display: flex;\n    align-items: center;\n    justify-content: center;\n    height: 200px;\n    font-size: 14px;\n    background: color-mix(in srgb, var(--st-text-1) 15%, transparent);\n    color: var(--st-text-1);\n    border: 1px solid var(--st-color-neutral);\n    border-radius: 4px;\n    margin: 20px 0;\n}\n\n.filter-type-toggle {\n    margin: 5px 0;\n}\n\n.toggle-btn {\n    padding: 4px 8px;\n    border: 1px solid #ddd;\n    background: #f5f5f5;\n    cursor: pointer;\n    border-radius: 4px;\n}\n\n.toggle-btn.active {\n    background: #e0e0e0;\n    border-color: #ccc;\n}\n\n.values-filter {\n    margin-top: 8px;\n    border: 1px solid #ddd;\n    border-radius: 4px;\n}\n\n.values-list {\n    max-height: 200px;\n    overflow-y: auto;\n}\n\n.values-list-header {\n    padding: 8px;\n    border-bottom: 1px solid #ddd;\n    background: #f5f5f5;\n}\n\n.values-search {\n    width: 100%;\n    padding: 4px;\n    margin-bottom: 8px;\n}\n\n.values-actions {\n    display: flex;\n    gap: 8px;\n}\n\n.values-actions button {\n    padding: 2px 6px;\n    font-size: 12px;\n}\n\n.values-options {\n    padding: 8px;\n}\n\n.value-option {\n    display: flex;\n    align-items: center;\n    gap: 8px;\n    padding: 4px 0;\n    cursor: pointer;\n}\n\n.value-option:hover {\n    background: #f5f5f5;\n}\n\n.field-header {\n    display: flex;\n    align-items: center;\n    justify-content: space-between;\n    margin-bottom: 8px;\n}\n\n.field-name {\n    font-weight: 500;\n}\n\n.value-label {\n    overflow: hidden;\n    text-overflow: ellipsis;\n    white-space: nowrap;\n}\n\n.values-filter {\n    margin-top: 8px;\n    border: 1px solid #ddd;\n    border-radius: 4px;\n    background: white;\n}\n\n.values-list {\n    max-height: 200px;\n    overflow-y: auto;\n}\n\n.values-list-header {\n    position: sticky;\n    top: 0;\n    padding: 8px;\n    border-bottom: 1px solid #ddd;\n    background: #f5f5f5;\n    z-index: 1;\n}\n\n.values-search {\n    width: 100%;\n    padding: 6px;\n    border: 1px solid #ddd;\n    border-radius: 4px;\n    margin-bottom: 8px;\n}\n\n.values-actions {\n    display: flex;\n    gap: 8px;\n}\n\n.values-actions button {\n    padding: 4px 8px;\n    font-size: 12px;\n    border: 1px solid #ddd;\n    border-radius: 4px;\n    background: white;\n    cursor: pointer;\n}\n\n.values-actions button:hover {\n    background: #f0f0f0;\n}\n\n.values-options {\n    padding: 8px;\n}\n\n.value-option {\n    display: flex;\n    align-items: center;\n    gap: 8px;\n    padding: 6px 8px;\n    cursor: pointer;\n    border-radius: 4px;\n}\n\n.value-option:hover {\n    background: #f5f5f5;\n}\n\n.value-option input[type=\"checkbox\"] {\n    margin: 0;\n}\n</style>"]}, media: undefined });

    };
    /* scoped */
    const __vue_scope_id__ = "data-v-86206f80";
    /* module identifier */
